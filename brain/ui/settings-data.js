  ...
  categories: [
      ...
      sections: [
        {
          ...
          rows: [
            ...
            { type: 'apikey', key: 'api_key_google',     label: 'Google (Gemini) API key',  hint: 'optional — image processing + Google voice (Cloud TTS/STT)', def: '' },
            { type: 'apikey', key: 'api_key_google_maps', label: 'Google Maps API key',      hint: 'optional — world-grounding tools (places, weather, directions); enable in Motor Permissions', def: '' },
            { type: 'apikey', key: 'api_key_google_vertex_sa', label: 'Vertex service-account JSON', hint: 'optional — paste the full service-account JSON to use Vertex AI on a hosted pod (local uses gcloud ADC instead)', def: '' },
          ],
        },
        {
          ...
          rows: [
            { type: 'select', key: 'cloud_provider', label: 'Reasoning', hint: 'which LLM serves the reasoning path — drafting, understanding, idle thought. Vertex runs the SAME Claude models via Google (billing + auth move to Google Cloud; needs Vertex enabled + ADC/service-account auth)', def: 'anthropic',
              options: [ { v: 'anthropic', l: 'Anthropic (Claude)' }, { v: 'openai', l: 'OpenAI (GPT)' }, { v: 'vertex', l: 'Vertex AI (Google-hosted Claude)' } ] },
            { type: 'toggle', key: 'enable_vertex', label: 'Enable Vertex AI', hint: 'allow Vertex AI (Model Garden: Gemini + Claude on Google) — auth via service account / ADC, not an API key', def: 0 },
            { type: 'text', key: 'vertex_project', label: 'Vertex project', hint: 'GCP project id for Vertex (blank = GOOGLE_CLOUD_PROJECT env)', def: '' },
            { type: 'text', key: 'vertex_location', label: 'Vertex region (Gemini)', hint: 'e.g. us-central1', def: 'us-central1' },
      ...
      sections: [
        ...
        },
        {
          ...
          rows: [
            ...
            { type: 'select', key: 'dmn_isolated_roster', label: 'Idle roster (isolated orgs)', hint: 'which personas share the idle loop when the org’s learning mode is isolated — home only (purchase personas never think idle), home + personas a human has talked to recently, or every full-tier persona; cadence thins with the count; consolidated orgs ignore this', def: 'active', adminOnly: true,
              options: [ { v: 'home', l: 'Home persona only' }, { v: 'active', l: 'Home + recently active personas' }, { v: 'all', l: 'Every full-tier persona' } ] },
            { type: 'num', key: 'dmn_active_roster_days', label: 'Active persona window (days)', hint: 'a persona counts as active — and keeps its idle thinking on the shared loop — while a human has taken a turn with it in the last N days; 0 = every persona counts as active', def: 7, adminOnly: true },
            ...
            { type: 'toggle', key: 'content_read_policy', label: 'Content Read Policy', hint: 'console and owner-key reads of learned content (transcripts, jobs, approvals, thoughts, living self-model, user-model) follow the org’s learning mode: consolidated = org admins only, audited; isolated = never for a non-home persona. Dials, the Seed and chemistry are never gated. Off = the pre-2026-09-13 reads (kill switch only; reads are still audited)', def: 1, adminOnly: true },
        ...
        },
        {
          ...
          advanced: [
            ...
            { type: 'toggle', key: 'fragment_explore_relevance_ranked', label: 'Relevant Exploration', hint: 'draw the explored fragment from those most similar to the current input instead of a blind roll. Off = legacy random roll.', def: 1 },
            { type: 'range', key: 'fragment_explore_top_k', label: 'Relevance Top-K', hint: 'how many of the most input-similar fragments the exploration roll draws from', min: 1, max: 8, step: 1, def: 3 },
            { type: 'group', label: 'Stance draw' },
            { type: 'toggle', key: 'stance_chem_affinity', label: 'Chemistry Affinity', hint: 'chemical state biases which information posture is drawn (stress toward caution, drive toward action, vigilance toward verification).', def: 1 },
            { type: 'toggle', key: 'stance_chem_complexity', label: 'Cognitive Economy', hint: 'chemical state biases method depth (a stressed brain reaches for the fast heuristic, a calm motivated one can afford depth).', def: 1 },
            { type: 'range', key: 'stance_draw_floor', label: 'Stance Floor', hint: 'minimum draw probability every stance keeps — chemistry biases the draw but can never make a stance unreachable', min: 0, max: 0.1, step: 0.01, def: 0.02 },
    ...
    /* ===================== AUTONOMY & MAINTENANCE ================= */
    {
      ...
      sections: [
        ...
        {
          id: 'sec-m4', num: 'M4', title: 'When You Direct It',
          desc: 'Grants for work that executes a live command from you. The same four dials exist in both columns — set each column as loose or as tight as you like.',
          rows: [
            { type: 'toggle', key: 'motor_user_writes', label: 'File Writes', hint: 'allow file writes and mutating shell commands when executing your requests — off limits even commanded work to reading, listing, and searching', def: 1 },
            { type: 'toggle', key: 'motor_user_network', label: 'Network Fetch', hint: 'allow fetching public web pages when executing your requests', def: 1 },
            { type: 'select', key: 'motor_user_cloud', label: 'Cloud Actions', hint: 'connected-service access when you command it — writes always ask you to confirm first either way', def: 'full',
              options: [ { v: 'full', l: 'Full (writes still confirmed)' }, { v: 'ro', l: 'Read-only' }, { v: 'off', l: 'Off' } ] },
            { type: 'text', key: 'motor_user_connectors', label: 'Allowed Connectors', hint: 'one connector name per line (e.g. github) — when set, even commanded cloud sessions are built with ONLY these services. Empty = all configured connectors.', rows: 3, placeholder: '', def: '' },
          ],
          advanced: [],
        },
        {
          id: 'sec-m5', num: 'M5', title: 'When Acting On Its Own',
          desc: 'The same dials for work the brain initiates itself — idle-thought tasks and jobs recovered after a restart. Defaults are deliberately tighter: a read-only researcher unless you loosen it.',
          rows: [
            ...
            { type: 'select', key: 'motor_self_cloud', label: 'Cloud Actions', hint: 'connected-service access for self-initiated work — read-only lets it look things up but never act', def: 'ro',
              options: [ { v: 'full', l: 'Full (writes still confirmed)' }, { v: 'ro', l: 'Read-only' }, { v: 'off', l: 'Off' } ] },
            { type: 'text', key: 'motor_self_connectors', label: 'Allowed Connectors', hint: 'one connector name per line (e.g. github) — when set, self-initiated cloud sessions are built with ONLY these services; everything else simply doesn’t exist for that task. Empty = all configured connectors.', rows: 3, placeholder: 'github', def: '' },
          ],
          advanced: [],
        },
        {
