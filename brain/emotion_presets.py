...
REACTION_TAGS: list[str] = [
    "laughs", "laughs softly", "chuckles", "sighs", "exhales",
    "clears throat", "hesitates", "stammers", "scoffs", "gasps",
...
REACTION_TAG_RE = _re.compile(
    r'\s*\[(?:' + '|'.join(_re.escape(t) for t in sorted(REACTION_TAGS, key=len, reverse=True)) + r')\]',
    _re.IGNORECASE,
)
