    ...
    *,
    ...
    async def list_personas_route(
        ...
        if _p._index_read_on():  # noqa: SLF001 - same package
            ...
            # The index can narrow by an explicit allowlist and by template; a key
            # carrying prefix pins cannot be expressed as an IN list, so it takes
            # the scan path below (where _persona_allowed evaluates every pin).
            allowed = None
            indexable = True
            if _key_restricted(ctx):
                entries = [str(a) for a in (ctx.get("allowed_agents") or []) if a]
                pins = [_api_auth.parse_pin(a) for a in entries]
                plain = {
                    e.split(".", 1)[0] for e, pn in zip(entries, pins, strict=True) if pn is None
                }
                has_prefix = any(pn is not None and pn[0] == "prefix" for pn in pins)
                has_template = any(pn is not None and pn[0] == "template" for pn in pins)
                if has_prefix or (has_template and (plain or not template)):
                    indexable = False  # every pin is evaluated on the scan path
                elif plain:
                    allowed = sorted(plain)
                # else: a single template pin → the template scope set above
            if indexable:
                try:
