...
**Status.** Live, both flags on. Honest about where it stands: this is the newest system in the document and it went live without soak time. Three known problems are being fixed as this is written. Its self-generated inferences are stamped with the same source label as genuinely external verdicts, which flatters the self-grading measure in §4.3 rather than respecting it. One of its three cues is true on almost every turn, so it arms far more readily than intended and flags ordinary topic rotation as avoidance. And a committed belief has no age-out, so it can pin itself in place while the steering it causes suppresses the very re-surfacing that would refute it. The mechanism is sound and the calibration is not there yet. Do not describe this one as settled.
...
**One caveat, stated plainly, because this endpoint is the one place an outside party writes into the reward system.** As shipped, the route checks that the caller owns the session but does not check that the graded turn belongs to that session, nothing bounds the number of grades one turn can accept, and a grade naming a turn that does not exist still moves chemistry. So a partner could in principle move another partner's learning, and a repeated grade could pump dopamine past what a single write is allowed to. Three fixes are in flight as this is written: resolve the turn only within the caller's session and bind the reward to the personality that actually ran it, make one turn's chemistry-moving grade idempotent so later grades update the record without paying again, and refuse to nudge at all when the turn cannot be found. **Do not point partner traffic at this endpoint until those land.** The per-write clamp described above is real and holds; what is missing is a bound on the sum.
...
| **Avoidance reading** | Live but not calibrated. One of its three cues is true on nearly every turn, so it arms on ordinary topic rotation, and a committed belief has no age-out. Both being fixed. The mechanism is right; the thresholds are not. |
