...
The first level runs before any tool fires: candidate *approaches* to the turn compete — rival positions on whether this moment needs the outside world and how the problem should be attacked (§2.10). That is the content-fixation level, the one Dennett's model is actually about. The second level competes on the reply itself: five drafters with different dispositions write in parallel. A critic scores them. An empathy critic can veto. Then the brainstem waits for quiet, meaning a moment passes with no new draft arriving, and commits the best surviving one.

The split matters because the two levels disagree about different things. Approach candidates disagree about *what to do* — answer from memory or go to the world, commit or ask first. Drafters disagree about *how to say it*. Commit points punctuate the flow (the approach commits, the instruction commits, the articulation gate commits), so this is Dennett punctuated by commitments rather than Dennett pure — but the drafts genuinely disagree about content, not just wording.
...
Drafters also carry **stances** — per-turn thinking postures drawn from the stance library (§2.10) on separate injection budgets from their learned skill attachments — so two drafters attack the same input differently, not just in different registers. The stance a drafter explores is chosen by relevance to the current input (cosine against the turn's embedding), by learned weight, and by chemistry.
...
**Status.** Live, both flags on. Three of its design details exist because an adversarial review demanded them, and each guards against a flaw of the kind that hides: arming requires genuine dodge evidence, never mere staleness, so ordinary topic rotation cannot read as avoidance; no committed belief outlives its age-out, so nothing pins itself in place; and the reward path labels its own inferences as intrinsic rather than external, gates their payout on an informativeness base rate measured per persona, and pays from a shared per-turn budget so resolving several beliefs in one turn cannot multiply the ceiling.
...
Do not say "storage is free" or "forever." Storage is cheap relative to the cost of deciding what to keep, which is the actual argument, and it is not free. One bound now exists, and only one: the idle thoughts the DMN writes to itself age out after `dmn_idle_retention_days` (default 30). Conversations, agent runs and sleep insights are still kept without limit. See Appendix A.
...
Meaning becomes vectors on a dedicated CPU box first and our GPU second. There is no cloud fallback, by design: a second provider is a second vector space, and a stored vector can only be compared to vectors from the same model. The dedicated box exists so that remembering things stops depending on the GPU being awake. It is sized for what it does, a few short embeds per turn: one request slot and a small fixed thread count (`BRAIN_EMBED_SIDECAR_THREADS`, default 2), sent with every request. Left to size itself it took every core it could see and became the largest line on the hosting bill. Any vector of the wrong shape is rejected loudly rather than stored quietly, because the failure mode it guards against is silent: the wrong model gets pulled, everything appears to work, and recall slowly stops finding anything.

That rule was learned the expensive way. For three months embeddings came from Google; from 2026-09-13 they came from the CPU box. Both produce 768 numbers, nothing recorded which had produced a given row, and so every search ranked half of memory by a distance that means nothing — quietly, with no error anywhere. Each row now carries the model that embedded it (`episodes.embed_model`) and the search functions only compare rows from the model asking. `scripts/reembed_episodes.py` converts what is already stored.

When the chain is unreachable, the router says which host failed and why — once per outage, not once per call — and embeds are skipped for a cooldown (`embed_local_retry_s`) rather than each one paying the per-host timeout. A turn during an outage is stored unembedded (a NULL vector, never a zero vector, which pgvector can never return from a cosine search) and the repair script fills it in later. A dedicated box gets a keepalive embed every `embed_sidecar_keepalive_s` (default 60 s), so its model never goes cold and the moment it answers the cooldown ends.
...
The passes that used to scan every personality on disk (learning stories, angle synonyms, reflex mining) are now bounded to the personalities whose turns are in the batch, plus home (`sleep_scan_all_personas: 1` restores the scan). And the one consolidation write the privacy gate never covered — the self-model rewrite and the inner-life digest, which absorbed every customer's specifics into the personality's autobiography — now passes through the same gate when a batch carried partner customers in a consolidated org, and is dropped rather than written when the gate refuses (`self_model_deid`).
...
It is the newest of the prospective-memory surfaces, built because idle thoughts had no durable home. The write path is covered by an end-to-end test that drives a real thought through a real store to disk. The section comes into existence the first time a persona opens a thread; as of the audit that had not yet happened in the live file.
...
The pin is enforced at the reward path itself, not the persona table: the Stoic is short-circuited to flat before any layered value — a settings override, an assigned mandate's own weights — can multiply through onto it. The composite of those layers is also clamped at both ends, so no combination of a persona's leaning, an override, and a mandate can drive a source close enough to zero to silence it or high enough to swamp the others.
...
### 9.4 Placing a personality (the premium tier) · Live

A personality can be **placed** on a dedicated brain of its own. The placement is a row the org writes through the owner API — mode, which GPU the instance talks to, whether it stays up, until when — and the gateway's desired-state loop makes the processes and pods match the rows: on the write itself (the API nudges the gateway), on every event that can change the answer, and at a slow resync otherwise. Nothing about isolation changes: state lives at an organization-canonical path for shared and dedicated alike, because a per-instance path would **fork the personality's mind on promotion**, splitting its ledger, its stories, and its chemistry in two. A personality thinks in exactly one place. Never both, never neither: the shared brain refuses to bind a personality that has its own instance, and the placement file it reads is derived from live processes, so it self-heals when one dies.
...
The loop itself (`brain/gateway/placement_control.py`) runs inside the gateway's reconcile tick, before the pool step. It reads every org's rows in one service-role query (re-read on a placement or budget event, otherwise cached; a failed read serves the last-known rows, so a database blink never reads as a mass revocation), spawns the missing dedicated instances in the background under the org's caps, pins the `always_on` ones past the idle reaper, and consolidates-then-stops any dedicated instance whose row is gone or expired. Reaching a dedicated instance is by header (§28): the header is honoured only for a persona that already has one, and never starts a process. `BRAIN_MULTI_PERSONA=0` is the one kill switch for the loop and the routing together, so a dedicated process nobody can route to cannot exist. A superadmin reads the loop's view at `GET /__fleet/placement`: pods, their state and consumers, refused spawns, orgs whose GPU budget is spent.
...
**The reconciler is level-triggered, woken by edges.** Neither the pool nor the placement loop runs on a timer any more (`brain/gateway/reconciler.py`). Each tick is an idempotent desired-state function, so it does not matter why it runs; what matters is *when*. It runs when a tenant nudges the gateway over a loopback, token-gated route — a placement row written, a consumer asking for its pod, output arriving, pressure changing — when a child process exits, when an org sleeps, and at the earliest moment the state machine itself needs one: the end of a pod's grace period, a paid_until expiry, a scale dwell, a churn cooldown, an org's projected budget exhaustion, the UTC rollover that refills a budget. Wakes within a second coalesce into one tick. The database is an edge source too: a trigger on the placement rows and the org caps posts to the gateway through `pg_net` (migration 042, configured from Vault), so a row edited in the dashboard or by a script wakes the loop the same as one written through the API. A fifteen-minute resync is the only periodic work left, there so a lost nudge self-heals, the way a Kubernetes controller resyncs its informers. Billing needs no clock of its own: uptime is measured between ticks whenever they happen.

**The GPU** is a pool, not a pod. Every brain process on the deployment joins one queue, and the pool holds between zero and a small ceiling of identical pods to serve it. Pod 0 comes up on demand and only for agents that need it, so a lite-only host never spins one; it stays up only while something is actually getting output back, not merely asking. Pods above it are added on pressure and removed on idleness. The pressure signal is measured, not guessed: every process records how long each local call waited for one of its concurrency slots, how long the pod took, and whether anything came back, and publishes that once every half minute. The gateway folds those samples onto the pod each process is assigned to, so a pod's utilisation is the busy seconds of its own consumers against the request slots it actually has. When a pod stays hot, or its callers keep queueing, for five minutes, another pod comes up; when a pod above zero sits idle for fifteen, its consumers are moved to the least-loaded pod, it drains for a moment, and it goes. Assignment is sticky: a process keeps its pod until that pod dies or is drained, so a tick never reshuffles the fleet. A dead pod's consumers are reassigned the same tick. One daily dollar ceiling covers the whole pool, billed per held pod at the highest rate among them; when it is spent nothing is added and the largest slot sleeps first. The ceiling is a platform value, not a tenant one, and it lives where a platform admin can reach it: the gateway reads a small runtime file on the shared volume, written from the Fleet console's "Platform GPU budget" card (or `PUT /__fleet/pod_budget`, superadmin only, audited), and falls back to the bundled settings default only when no one has set it — runtime file first, bundled settings second. It is re-read on the reconciler's next tick, so changing it is not a deploy. Zero means uncapped, and the console and the route both say so out loud.

That queue is also the honest statement of the earlier bug: the concurrency limit meant to bound calls onto the pod was not being taken on the streaming path, so saturation looked like a quiet system rather than a queue. It is a queue now, and the queue is what the pool scales on.

Five mechanisms keep a pod from thrashing, each guarding a failure that looks like success. A residency check, because a GPU can answer health checks while silently running on CPU at a crawl. A cooldown after a failure, so a systemic fault cannot churn create-and-destroy every minute. An expiry on that cooldown, so a transient blip does not blacklist a good machine for the life of the process. A rule never to unload the model, because it bills the same either way and an unloaded model looks absent, which triggers the very churn the rule exists to prevent. And a memory floor, because a large model on a small card is a silent, expensive nothing. The pool adds two of its own: a dwell time in both directions, because a hot minute is not a trend, and a refusal to drain a pod whose load would push the rest of the pool back over the threshold, because that is how oscillation starts.
...
**Observability** is per-call tracing that self-disables without keys, one unified decision channel, and a per-personality learning ledger that fixes the container rather than the content. It is populated centrally, so **no learning code knows the ledger exists.** Nothing to forget to call. Two of these channels are process-wide and append-only, which is exactly what right-to-erasure cannot reach — so they are handled at the write, not the delete: the eval log stores a partner customer's prompt and response as digests (and lives under each org's own root on a multi-tenant host, no longer one file shared by every tenant process), and the tracing export skips partner turns unless the deployment has opted in and named the tracer as a sub-processor. What the platform stores, where, and for how long is written down for partners in the API guide's data-handling section.

### 9.13 Learning mode (consolidated vs isolated personalities) · Live

**The unit of learning is the personality, and the client chooses its cardinality.** Everything a personality learns — wiring, self-model, stories, reflexes, stances, resting mood, idle-thought state — is already keyed by personality. So the question a companion marketplace asks ("every purchase its own brain, nothing learned in one may reach another") is not a question about processes; a process is a placement decision. It is a question about how many individuals exist. An org-level account setting answers it (`organizations.learning_mode`, read by every process of the org through a short cache so the two shapes of instance can never disagree):

**Consolidated** (the default, and the thesis): one personality does a job for the client and meets many people in it. It learns across all of them, the way an employee does, and what one customer teaches it may shape how it treats the next — through the privacy gate in §9.11, never verbatim. Each customer keeps a private silo (memory recall, profile, mood pair, approvals), and inside a personality three engine-lane read paths that used to carry one customer's words into another's turn (structural recall, the profile grep, the idle memory seed) are now scoped to the bound customer.
...
### 9.13a What the org can see of an isolated personality (read-path policy) · Live

**The learning mode decides who may read learned content, not only who may learn from it.** Every console and owner-key read of a personality's *learned* material — engine-lane transcripts, background jobs (request, steps, output), approval payloads, idle thoughts, the living self-model, the model of the people it talks to — passes one decision (`brain/read_policy.py`). In a consolidated org that content is the org's own work product: org admins may read it (members may not), and every read is written to the governance log with the kind, the personality and the route, never the words. In an isolated org each non-home personality is one buyer's companion, so the same reads answer "not available" or a content-free projection (states, counts, costs, lengths, timestamps) for everyone, the platform operator included; there is no break-glass. The org's home personality, which buyers never bind, follows the consolidated rule on its owner lane. Configuration is not content: a personality's dials, its authored Seed, and its chemistry (resting and current mood) stay readable and editable in both modes, because they are what the operator set, not what the buyer said.

**The lane stamp is an authorization filter, not a paint hint.** Console clients receive agent-lane events and idle thoughts in full or projected per client, decided at the server; the connect-time replays obey the same rule; a thought carries the personality that thought it so a buyer persona's musings never reach the owner feed in an isolated org. Process logs digest a slice of any goal, reason or summary produced on the engine lane or inside a non-home binding of an isolated org; the eval log digests the customer's identity and the drafter's prose along with the turn text.

**The operator's view is the fleet, not the conversations.** The Fleet console lists every personality with what an operator needs to run a marketplace of thousands — last human turn, roster membership, learned-state, agents, cost, open and stuck jobs, health flags — searched and paged in O(page) from the personas index (migration 039), with a per-personality card (dials, chemistry, projected jobs, fingerprint history), org health (idle loop, roster cadence, breaker, GPU budget, capacity), spend by partner key, and the governance log. Buyer identities appear only as a keyed hash for support correlation; `GET /v1/personas/{p}/isolation` reports how many admin content reads a personality received in 30 days, which is zero by construction in an isolated org. Memory is bounded the same way the loop is: the least-recently-bound personalities beyond a cap, or idle past an hour, are persisted and dropped from the process, the roster and open sessions excepted.

**The index is rebuilt from the console, not from a key.** The personas index is kept in step by every write, but an org that existed before migration 039, or whose volume was restored, holds missing or stale rows (zero turns, no agents, no last human turn) until someone runs a full rebuild — and the owner route for that (`POST /v1/personas/reindex`) needs the org's owner key, which the platform operator does not hold. Fleet → All orgs carries a Reindex button per org and a Reindex-all-live button; Fleet → Org health carries one for the org being viewed. Behind them the gateway (`POST /__fleet/orgs/{org_id}/reindex`, `POST /__fleet/reindex_all`, platform admin only) asks the org's live brain to rebuild its own index over a per-boot internal token the provisioner hands every tenant (`BRAIN_INTERNAL_TOKEN`, presented as `X-Brain-Internal-Token`; the token admits only the gateway-only paths, never a read). A single org that is asleep is booted for the rebuild, the same path a login takes; the all-orgs sweep skips asleep orgs and reports them, unless asked to boot them (`?spawn=1`). Every answer is counts and seconds — rows indexed, personas with learned state, elapsed — and the cross-org counts are re-read on the next refresh.

### 9.14 The pod pool and standalone pods · Live

**At the basic tier every personality shares one queue, and the queue's GPU scales.** Local model calls from every brain go to a pool of identical pods behind a single published host list; each brain reports its pressure (in-flight calls, waits, saturation) to a small file every half-minute, and the gateway scales the pool from that signal — up when a pod is busy or waits grow and stay grown, down when a pod has been idle long enough to drain and hand its consumers to a sibling. When the pool is at its ceiling, calls **queue**; they never fall to the cloud, because the cloud is a different price and a different promise. A platform-wide daily budget bounds the pool the way the single pod was bounded before; when it is spent, nothing scales up and the largest pod sleeps first. That budget is set at runtime by a platform admin (§9.12), not baked into a deploy.
...
**How the controller holds a pod of the org's own.** Each standalone or org pod is its own pod manager with a deterministic name, so a redeployed gateway adopts it rather than creating a twin. It wakes when a consumer's pressure file shows fresh demand, is held while a consumer is getting output, and is paused after the pool's grace period — pod 0's contract, per pod. Before any of that, two gates: the org's daily GPU budget (zero means no pod at all; spent means the org's pods sleep until UTC rollover and its instances ride the pool) and a platform backstop on how many dedicated pods may be held at once (`BRAIN_MAX_STANDALONE_PODS`). A dedicated pod is published to consumers only once it is *ready*; until then, and whenever it cannot be created, the instance stays on the pool and the placement's live view says `fallback_pool` with the reason. A consumer served by its own card no longer touches the platform-wide demand and use files, so a persona thinking on its own GPU never keeps the shared one awake. An org's Sleep pauses its pods with its instances.
...
**Verdict: Solid, and honestly bounded.** Deliberation runs at two levels — the approach competition on content-fixations before any tool fires (§2.10), the drafters on phrasing after (§2.4) — so the drafts genuinely disagree about what to do, not merely how to word it, which is the part of Dennett most implementations miss. What is deliberately un-Dennettian: commit points exist (the approach commits, the instruction commits, the articulation gate commits). Claim "multiple drafts at two levels, punctuated by commitments" — do not claim there is no place where a decision happens, because there are three, and they are named.
...
**Verdict: Solid architecturally, with an acknowledged inversion.** Encoding is deliberately *indiscriminate*, the opposite of the classical account. Storage is cheap relative to the cost of deciding what to keep; retrieval is the intelligence. All selectivity moved to consolidation and retrieval, where hindsight is available. Missing: no replay, no interleaved rehearsal, no forgetting mitigation. Forgetting now exists in exactly one place — idle thoughts age out (`dmn_idle_retention_days`) — and nowhere else.
...
*Last audited 2026-07-19. What remains here is either a real limit, a deliberate position, or a decision waiting on you. Appendix C records what past audits changed.*
...
| **Memory retention** | Decided for one class of row, 2026-09-20, and still open for the rest. The DMN's own idle output — the deferred questions and conclusions it writes while nobody is talking — ages out after `dmn_idle_retention_days` (default 30), deleted at the end of consolidation. It earned the exception by volume: in production it outnumbered real turns roughly ten to one, and the DMN's own dedup already suppresses much of it as repetition. Everything a person or a customer actually said is still kept without bound, and the only other deletion path is erasure on request (`api_purge_end_user`, plus `purge_source` for hypotheses). Growth is linear in substantive turns, per persona, per tenant, without bound. One developer's local store is already ~49 MB across a handful of personas. `CONSTITUTION.md` still reads *"Nothing is forgotten unless explicitly pruned by the user. This is a genuine transcendence of a human limitation"*, and the public site sells perfect recall. Idle thoughts are the entity's own musings rather than anything it was told, so ageing them out is arguably outside that promise — but the wording has not been revisited and **should be**, before any broader retention rule is considered. Do not claim "storage is free" or "forever" anywhere. |
