...
**The empathy check itself now runs on the local machine too, and that is a separate decision from the experiment.** It answers a narrow question — would this reply read as insensitive to someone feeling this way — and it runs once for every draft, so it was the largest paid-per-turn judgement in the system and the least open-ended. The main critic, which scores craft and coherence and is what draft selection actually turns on, stays on the cloud model. But the empathy cell holds a veto, and a cell that can stop a reply must not go quiet when the local machine is down. So it prefers local and falls back to the cloud, which is the opposite of how the experiment behaves, and the asymmetry is the point: a missed experiment costs some learning, a missed empathy check ships the reply.

That surfaced an older problem worth naming. When the check failed to return anything usable, the code substituted a passing score — a fabricated approval that cleared the veto and then fed the blended score, the appraisal stream, and the accuracy record as though a real judgement had happened. It was already wrong when a cloud call timed out, and it would have become routine on a smaller local model. A check that does not return now reports nothing at all rather than approval, which is how this system already represents every other appraisal that did not run.
