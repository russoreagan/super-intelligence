...
| `BRAIN_MAX_PERSONAS` | `5000` | call | Per-org ceiling on CUSTOM persona specs (built-in overrides excluded). `POST /v1/personas/{template}/clone` returns 409 at the cap; reported as `limits.max_personas` on `GET /v1/personas`. A clone costs no process (it binds per turn on the shared brain), so this bounds catalogue size, not compute. `0` = uncapped. `brain/personas.py:capacity_limits` |
