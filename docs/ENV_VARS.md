...
| `BRAIN_MULTI_PERSONA` | `1` (on) | import ⚠ | The premium tier's one switch: the gateway honours the `X-Brain-Persona` header on the engine-API lane — only for a persona that already runs on its own dedicated instance (`provisioner.promoted_personas`); any other value routes to the org's shared instance and the header never spawns a process — AND the placement desired-state loop (`brain/gateway/placement_control.py`) that creates those instances from `persona_placement` rows. `0`/`false`/`no` disables both together. An org with no placement rows is unchanged either way. The owner UI always talks to the tenant's default process. `brain/gateway/server.py` |
...
| `BRAIN_MAX_STANDALONE_PODS` | `4` | call | Platform backstop on dedicated GPU pods (standalone + org) held at once, above every org's own `gpu_daily_usd_budget`. A placement past it stays on the pool (`fallback_pool`, reason `platform_cap`) until a pod frees. `0` = uncapped. `brain/gateway/placement_control.py` |
| `BRAIN_MAX_DEDICATED` | `3` | import ⚠ | Deployment DEFAULT for the per-org ceiling on DEDICATED persona instances (Path A `org::persona` spawns); an org row's `organizations.max_dedicated_instances` (migration 038) overrides it when > 0. The resource they multiply is the shared GPU pod (each runs its own DMN against one serialized 32B), not Railway RAM. `0` = uncapped. At cap the persona stays on the shared instance via per-turn binding and `POST /v1/personas/{p}/placement` answers 409. `brain/provisioner.py`, `brain/persona_placement.py` |
| `BRAIN_MAX_PERSONAS` | `10000` | call | Per-org ceiling on CUSTOM persona specs (built-in overrides excluded). `POST /v1/personas/{template}/clone` returns 409 at the cap; reported as `limits.max_personas` on `GET /v1/personas`. A clone costs no process (it binds per turn on the shared brain), so this bounds catalogue size, not compute. The cap check is a head count on the persona index (migration 039) when it answers, else a spec scan. `0` = uncapped. `brain/personas.py:capacity_limits` |
| `BRAIN_OWNER_REF_SECRET` | `""` | call | HMAC key behind `owner_ref` — the content-free handle the fleet console shows for a persona's buyer (`hmac_sha256(secret, end_user_id)[:12]`, written to the `personas` index row on ownership claim). Unset = a per-org secret file `<tenant root>/.owner_ref_secret` is minted on first use (`secrets.token_hex(32)`, mode 0600). Set it when several processes must agree on refs without a shared volume. `brain/persona_owners.py:owner_ref_for` |
| `BRAIN_PERSONA_CHEM_RESIDENT` | `256` | call | How many BOUND (non-home) personas keep a live per-customer chemistry registry in memory at once; the least recently used is flushed to its persona's `client_chem/` and evicted. `0` = unbounded. `brain/session_turn.py:_persona_chem_registry` |
...
| `BRAIN_EMBED_SIDECAR_THREADS` | `2` | import ⚠ | CPU threads per embed request on the sidecar, exported to tenants as `OLLAMA_EMBED_NUM_THREAD`. The sidecar also runs one slot (`OLLAMA_NUM_PARALLEL=1`). Unbounded, Ollama sized its CPU runner to the host (48 cores visible on Railway, 32 in quota) and the sidecar averaged ~11 vCPU. `brain/gateway/server.py` |
| `BRAIN_OLLAMA_BIN` | `""` | call | Explicit path to the ollama binary for the embed sidecar; otherwise PATH, `/usr/bin/ollama`, `/usr/local/bin/ollama` are probed and the skip line names what was probed. The installer needs `zstd` (nixpacks `[phases.setup] aptPkgs`), which is why the sidecar silently never ran until 2026-09. `brain/gateway/server.py` |
| `BRAIN_TENANT_PLATFORM_KEYS` | `""` | call | Comma-separated provider slugs (`anthropic,google,deepgram,elevenlabs`) whose PLATFORM key a tenant process may inherit from the gateway env. Default: none — tenants are BYO-key for every provider, and an org with no Google key of its own no longer embeds on the platform's. `brain/provisioner.py` `platform_secrets_to_strip` |
...
| `RUNPOD_STANDALONE_VOLUME_ID` | `""` (ephemeral disk) | call | Network volume for the models of standalone / org pods — never pod 0's volume. Unset → each dedicated pod pulls its models onto an ephemeral disk at creation. `brain/gateway/placement_control.py` (`default_manager_factory`) |
...
| `BRAIN_DB_WEBHOOK_SECRET` | unset (off) | call | Shared secret for the database → gateway edge (migration 042): Supabase's pg_net trigger posts placement / org-cap changes to `POST /__nudge/db` with this value in `x-brain-webhook-secret`; the same value is stored in Vault as `gateway_nudge_secret` (with `gateway_nudge_url`). Unset = the route answers 404 and the trigger is a no-op; rows edited outside the owner API are then noticed at the resync. `brain/gateway/server.py` |
...
| _(file)_ `<BRAIN_TENANTS_DIR>/.pod_budget_config.json` | absent → bundled `pod_daily_usd_budget` (`10.0`) | call (mtime-checked each tick) | The platform pool's daily GPU ceiling at RUNTIME. Precedence: **this file > bundled `brain/settings.json`**. The gateway enforces the ceiling for every org and runs with no `BRAIN_SETTINGS_PATH`, so without this file the value is a deploy-time constant no admin can change. Written only by the superadmin route `PUT /__fleet/pod_budget` (Fleet → Health → "Platform GPU budget" card); `0` = uncapped (the route returns a `warning`). Every write is appended to `.pod_budget_audit.jsonl` beside it. Not a tenant setting: a tenant's `settings.json` value is never read by the gateway. `brain/pod_budget.py` (`runtime_budget_usd`, `_settings_budget_usd`), `brain/gateway/server.py` |
...
| `BRAIN_RUNPOD_POOL_FILE` | `` (injected at spawn: `<tenants>/.runpod_pool.json`) | call | The pod-pool file consumers read FIRST: `pods[]`, `assignments{proc_key→pod_id}`, `standalone{}`. A process it mentions follows its assigned pod (booting/draining/absent → the `off` sentinel); one it does not mention follows `BRAIN_RUNPOD_HOST_FILE` as before. Tenants also read it for the `live` block of `GET /v1/personas/{p}/placement` and the pool rate on `GET /v1/usage`. `brain/runpod_manager.py` (`_consumer_host`), `brain/pod_pool.py` (`resolve_pool_host`), `brain/placement_client.py` |
...
| `BRAIN_INTERNAL_TOKEN` | unset → minted per gateway boot | call | Gateway→tenant shared secret for the tenant's gateway-only routes (`INTERNAL_PATHS`: `POST /__reindex`, behind Fleet → Reindex). The provisioner injects it into every tenant's env; the tenant admits a request on one of those paths only when `X-Brain-Internal-Token` matches (constant-time, ≥ 16 chars). Set it to pin the value across gateway restarts; unset, a tenant spawned by an earlier gateway process stops answering internal calls until it respawns. `brain/provisioner.py:internal_token`, `brain/ui/auth.py:is_internal_request` |
| `BRAIN_FLEET_REINDEX_TIMEOUT_S` | `600` | import ⚠ | How long the gateway waits for one tenant's persona-index rebuild (`POST /__fleet/orgs/{org}/reindex`; `reindex_all` waits this per org, sequentially). A marketplace org with thousands of personas and `learned=True` counting can take minutes. `brain/gateway/fleet_orgs.py:REINDEX_TIMEOUT_S` |
...
| `BRAIN_EVAL_LOG` | `eval/turns.jsonl`; multitenant: `<org root>/eval/turns.jsonl` | call | Eval log path. On a multi-tenant boot `brain/run.py:_route_persona_state` defaults it under the ORG's second_brain root (setdefault — an explicit value wins), so tenant processes no longer share one repo-relative file. `eval/turn_logger.py:log_path`, `eval/report.py:246`, `eval/skill_judge.py:188`, `eval/identity_judge.py:177` |
