...
| `BRAIN_RUNPOD_POOL_FILE` | sibling of the host file (`tenants/.runpod_pool.json`) | call | The gateway's pool file (plan §10.2: `pods[]`, `assignments{}`, `standalone{}`). Tenants read it for the `live` block of `GET /v1/personas/{p}/placement` and the pool rate on `GET /v1/usage`; absent = legacy single-pod view from the host file. `brain/placement_client.py` |
