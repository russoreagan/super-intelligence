...
| `BRAIN_MULTI_PERSONA` | unset (off) | import ⚠ | Gateway routes per `X-Brain-Persona` header so one tenant can run several persona processes. `brain/gateway/server.py:67` |
...
1 hard duplicate-default conflict (`BRAIN_STT_KEYWORDS`; the
`BRAIN_SESSION_IDLE_TIMEOUT_S` conflict was resolved by deleting dead
`brain/session_manager.py`), plus the `BRAIN_USER_ID` `"dev"` divergence and the
