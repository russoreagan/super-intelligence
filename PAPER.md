...
**Russ O'Reagan**  
*Unpublished technical report, June 2026*

---

## Abstract

We describe the design, implementation, and empirical observations of a biologically-inspired cognitive architecture whose central hypothesis is that implementing the affective substrate of cognition — persistent emotional state that modulates computation rather than merely framing output — produces a system that develops differently over time. We frame the goal as **artificial emotional intelligence (AEI)** rather than artificial general intelligence (AGI): the design optimizes for affect as a load-bearing internal state that gates computation, not for breadth of task capability. The architecture maps explicitly onto regions of the human brain, with a two-tier chemical signaling system — five fast neuromodulators (dopamine, acetylcholine, GABA, glutamate, norepinephrine) and four slow hormones (serotonin, cortisol, oxytocin, anandamide) — coordinating a fixed set of heterogeneous, region-specific agents rather than a central planner. Language model calls are treated as expensive convergence-zone events, with the majority of per-turn computation delegated to cheap, deterministic switch neurons. The architecture draws from predictive processing theory (Friston, Clark), dual-process theory (Kahneman), Global Workspace Theory (Baars, Dehaene), Dennett's Multiple Drafts model, and Clark and Chalmers' extended mind hypothesis. Across an operational dataset of 393 turns over 69 sessions, the system holds its designed cost envelope (mean 3.38 LLM calls/turn against a 3–5 target), produces measurable predict-and-surprise gating saves on 25.4% of turns, and recalls long-term memory on 35.9%. The full nine-channel chemical system runs: the fast neuromodulators settle from early to late sessions as the conversational environment becomes familiar, the slow hormones hold coherent baselines (cortisol near its floor, oxytocin accumulating, serotonin stable), and the emotion model expresses fifteen distinct states including a first negative-valence base state. Beyond the brain's own chemistry, the coordination layer adds two mechanisms nature converged on independently for decentralized control — stigmergic signaling and division of labor from social-insect colonies (quorum and silence-as-signal, need-proportional recruitment, sensory-filter specialization, live trail reinforcement) and branching-ratio criticality control with arousal as its parameter from flock dynamics — adopted as convergent designs rather than biomimetic novelties. The system also runs deliberate emotional expression decoupled from chemical state, an autonomous default-mode network under a background resource budget, a self-directed multi-step task system, and a per-turn reasoning-framework selector over a self-reflection library of roughly 380 skills. The central learning claim is tested directly. All three Hebbian routing surfaces (drafter selection, switch evaluation order, recall fan-out) are closed-loop — each biases live computation and is reinforced by turn outcome — and a controlled divergence experiment confirms the load-bearing prediction: two instances given different experiential histories, then probed on identical inputs with memory wiped and chemistry pinned neutral, route those inputs differently, and the difference collapses under a frozen-wiring control (permutation p = 0.002). The effect is cleanest on the recall surface (≈9× separation over the frozen control) and modest in the network aggregate; a live within-session trail overlay produces a further, separable routing effect. Plasticity follows a graded, neuromodulator-scaled learning rule consistent with three-factor learning theory and the inverted-U of stress on memory. We report these results with their caveats — an amplified training rate, a synthetic probe set, small samples — and note that the central question of whether multi-agent structure improves on a single well-prompted model on raw task quality remains untested. We present the architecture, its philosophical commitments, and an honest account of what the data does and does not indicate.
...
It is useful to name the target directly. Where the dominant ambition in the field is **AGI — artificial *general* intelligence**, measured by breadth of capability across tasks, the goal here is better described as **AEI: artificial *emotional* intelligence**. The distinction is not rhetorical. An AGI program treats affect, if it appears at all, as a surface property layered on top of a reasoning engine — a tone applied to an answer. An AEI program inverts the stack: affect is the *substrate*, a persistent internal state that decides how much to attend, how confidently to conclude, what to remember, what to learn, and when to mobilize the whole system rather than answer glibly — with language reasoning invoked as one expensive resource that affective state gates. The bet is that an architecture organized around emotion as a real, computationally-load-bearing state — not around maximizing general task performance — is a distinct and under-explored design point, and that it is the more honest description of what this system is built to be. The AEI framing recurs in this paper wherever it clarifies a design choice; it is offered as a positioning, not a claim that the system is generally intelligent, which it explicitly is not.
...
**Collective dynamics in social insects and animal swarms** (2024–2025) informs the two collective-coordination layers described in §4.5.1–4.5.2. On the social-insect side, recent work reframes pheromones not as fixed releasers of behavior but as *cognitive modulators* that adjust a receiver's learning and salience through aminergic (neuromodulatory) circuits rather than the primary sensory pathway (Giurfa, 2025) — a direct parallel to this system's separation of content processing from chemical modulation. Honey-bee colonies coordinate without a central planner through summed chemical concentration as a form of quorum sensing, the *absence* of a signal as information in its own right (Carroll et al., 2025), dual releaser/primer effects from a single chemical blend (Mondet et al., 2025; Gryboś et al., 2025), and recruitment that scales collective defense to need across individuals with deliberately diverse response thresholds (Ramírez-Moreno et al., 2025). Ant-colony work adds the mechanisms the bee literature lacked: stigmergic trail reinforcement *on traversal* rather than in a nightly batch (Salman et al., 2024; Porras-Brenes & Amador-Vargas, 2024), the response-versus-satisfaction-threshold distinction in task allocation (Lynch, Wilson, & Dornhaus, 2024) and its maximum-entropy softmax generalization to multiple task types (Lynch & Pavlic, 2024), regulation that keys on a signal's *rate of change* and not only its level (Baltiansky et al., 2023), division of labor driven by *sensory* differentiation rather than threshold variance (Caminer et al., 2023), and the quorum-triggered shift from deliberation to commitment in collective nest choice (Chan et al., 2025). On the animal-swarm side, starling murmurations exhibit *scale-free correlations* arising from operation near a *critical point* (Cavagna et al., 2010), with interaction governed by a fixed number of topological neighbors rather than metric distance (Ballerini et al., 2008) — a finding the authors explicitly bridge to criticality in neural assemblies, which the brain's locally-coupled switch network instantiates.
...
**Intent detection (embedding-based, learned).** The fast-path *intent* gates — whether the user is referring to the entity itself (`self_reference`) or using it as a cognitive tool that recalls shared history (`epistemic_action`) — are detected by semantic similarity rather than a fixed keyword list. Each intent owns a small per-persona bank of exemplar phrasings; the input is embedded once by a local model (off the LLM critical path) and matched by cosine against the bank, so paraphrases a literal list would miss still fire ("going back to what we discussed", "what is it like to be you"). The bank seeds from a handful of canonical phrases and **grows with use**: on turns where the understanding integrator already runs, an intent it confirms that the embedding match missed is added as a new exemplar — so the bank converges on how a given user actually phrases these intents and the integrator is consulted less over time. The fire threshold is deliberately conservative; coverage comes from the learning loop, not a loose threshold. A literal seed match remains the fast path and the fallback when no embedder is available. This is the general mechanism toward which the curated affect lexicon above is being migrated — a learned, paraphrase-robust detector in place of a static word list.
...
The chemical signaling described so far is the brain's own answer to a general problem: how a system of many local units produces coherent global behavior with no central controller, coordinated only through a shared, slowly-varying field that every unit reads. The brain is not the only place in nature that faces this problem, nor the only place it is solved this way. Social-insect colonies coordinate thousands of individuals through shared, decaying pheromone fields and division of labor by response threshold; bird flocks achieve instantaneous, flock-wide response by poising themselves near a critical point. These are the same computational problem, and evolution has converged on the same family of answers independently in neural tissue, in insect societies, and in animal swarms. That convergence is the point of the two mechanisms below. They are not biomimetic novelties bolted on; they are extensions of the chemical-signaling tier adopted because a coordination pattern that arises independently in brains, colonies, and flocks is strong evidence of a *fundamental* design for decentralized coordination — one this architecture, being exactly such a system, has every reason to use. The source literature is simply slightly different from the neuroscience that grounds the rest of the system; the design status is the same.

### 4.5.1 Criticality control and chemistry trajectory

The first mechanism brings the network to its point of maximal responsiveness and gives the chemistry a sense of its own direction of travel — two ideas from the collective-dynamics literature on starling murmurations, returned to the substrate that literature itself points back to: a network of locally-coupled units. Cavagna et al. (2010) showed that starling flocks exhibit *scale-free correlations* — the behavioral change of one bird influences all others regardless of flock size — and argued this arises because the flock sits near a *critical point*, explicitly bridging the finding to criticality in neural assemblies. The brain's switch network is exactly such a system: thousands of locally-coupled units, no central controller, global behavior emerging from local interaction — in the flock, each bird tracking a fixed number of neighbors irrespective of distance (Ballerini et al., 2008). The same poised regime that lets a flock turn as one is the regime in which a salient input propagates undamped through the cluster network while noise stays local.

**The criticality observable.** A network is maximally responsive — a salient input propagates undamped while noise stays local — when it operates near criticality, conventionally indexed by the *branching ratio* σ (the mean number of descendant activations each firing triggers): σ < 1 is sub-critical (activity dies out — sluggish, literal, repetitive); σ ≈ 1 is critical; σ > 1 is super-critical (runaway cascades — incoherent). This is the same σ used throughout the neural-avalanche literature (Priesemann et al., 2014; Bellay et al., 2015), and there is broad evidence that cortex operates in this regime, with avalanche dynamics tracking behavioral scaling laws (Palva et al., 2013) and criticality conferring optimal computational properties (Hesse & Groß, 2014). The system estimates σ each turn from the firing path (Section 4.10), reconstructing cascade ancestry through the wiring graph — counting wiring edges whose source and target both fired, over the fired nodes that can propagate. Per-turn σ is noisy small-N, so it is smoothed over a rolling window and reported alongside the avalanche-size distribution; a small-N guard returns no estimate when too few propagating nodes fired.
...
**Threat without a predator (the framing that grounds this).** Flock and insect criticality is selected for by *predation*: the flock is poised because a falcon will eat the slow. An AI has no predator and no body, so importing biological "threat" would be cargo-cult — keeping the dial without the thing it was for. What transfers is not threat but the *functional invariant* threat is one instance of: **stakes × urgency × need-for-system-wide coordination** — the condition under which a system should mobilize its whole self rather than let one sub-process answer glibly. In this architecture that invariant is instantiated by the system's real information-processing stakes: prediction-error (surprise), cost-of-error and irreversibility, and internal conflict among sub-processes — the same signals that already drive norepinephrine and cortisol (Section 4.5 grounds the stress signal in surprise, not only a hostility lexicon). This is consistent with the locus-coeruleus account, where arousal is driven by *task utility* monitored by anterior cingulate and orbitofrontal cortex, not by survival threat (Aston-Jones & Cohen, 2005). Two honest boundaries follow. The embodied, metabolic half of cortisol — energy mobilization, fight-or-flight physiology — has no referent here and is not modeled; only the information-processing role transfers. And the claim is functional, not phenomenological: arousal and cortisol are real control *states* with real downstream effects on computation, not asserted feelings. One consequence is that an AI, paying no survival penalty for relaxing, can legitimately rest *more* sub-critical than a bird whose baseline is shifted up by chronic predation risk — which is precisely why the setpoint is arousal-modulated rather than fixed at the critical point.

### 4.5.2 Stigmergic coordination and division of labor

The neuromodulator system (§4.5) coordinates clusters through global scalar levels — a bloodstream. A second coordination layer adds what the bloodstream lacks: *stigmergic* signaling — indirect coordination through a shared, decaying signal field — and an explicit division-of-labor mechanism. If neuromodulators are the colony's bloodstream, this is its pheromone field. It is built by reusing existing machinery (the exponential-decay model from `Neuromodulators`, the `effective_threshold` modulation path, the centralized hormonal writer in the hypothalamus, the `mem.recall`→hippocampus route, and the decisions log); it introduces no parallel subsystems and no new firing logic. The mechanisms derive from two convergent natural sources — honey-bee chemical communication and ant-colony task allocation — and instantiate the same decentralized-coordination principle the rest of the architecture already runs on, applied to inter-cluster signaling.

**Topic concentration, quorum, and silence-as-signal.** The base bus is fire-and-forget: it fans out each message and retains no per-topic state, so a cluster cannot distinguish "this topic went quiet" from "this topic never spoke." This layer adds, for designated high-stakes channels, a decaying *concentration*: the summed, time-decayed strength (`+= msg.confidence`, capped, exp-decayed at `colony_conc_half_life_s`) of recent signal on a topic — the analog of a colony reading the aggregate concentration of a brood or alarm pheromone rather than any single emission (Carroll et al., 2025). Concentration yields two derived signals. *Quorum* (`bus.quorum`): an ARMED topic whose concentration is above threshold **or rising fast** is a collective-action trigger that no single cluster decided. *Silence-as-signal* (`bus.is_quiet`): a channel that was active and then decays to silence is itself informative — the disappearance of a once-present signal carries meaning, mirroring the colony's response to the *loss* of brood pheromone (Carroll et al., 2025). The hard part is not the accumulation but distinguishing meaningful silence from cold-start absence, so silence is governed by an explicit three-state machine — `UNARMED` (never crossed the arm threshold; its silence is noise) → `ARMED` (was active) → `QUIET` (was ARMED, then decayed below `colony_silence_floor`) — with a long zero-dwell disarming a topic back to `UNARMED` so a one-time burst long ago does not make permanent silence "meaningful." Only an `ARMED→QUIET` transition counts; `is_quiet` is never true from cold start. The first channel armed this way is threat detection (`affect.state`). *How it applies:* sustained or fast-rising threat trips quorum and mobilizes system-wide caution; threat that was present and then clears trips the quiet edge ("stand down").

**Rate-of-change in quorum.** Colony foraging regulation keys on a forager's load *and its rate of change*, a cheap local rule that yields colony-level regulation (Baltiansky et al., 2023). Accordingly `quorum` fires on `level ≥ threshold` **OR** `slope ≥ colony_quorum_slope_threshold`, where the slope is the rise in concentration per second since the previous sample (reusing the decay timestamps already stored). A fast-rising threat trips sooner than a slow accumulation reaching the same level — escalation is detected, not just magnitude.

**Releaser + primer in one message.** A single pheromone blend can carry both a *releaser* effect (immediate behavior) and a *primer* effect (slow physiological reprogramming) at once (Mondet et al., 2025). The base architecture splits these across mechanisms — a transient `Message` (releaser, ~30s TTL) versus the slow `HormonalState` channels (primer) that only the hypothalamus writes. This layer lets a `Message` carry an optional `primer: dict[str, float]`; the bus accumulates primer nudges on publish, and the hypothalamus drains them once per turn (`bus.drain_primers()`) and applies them through the single existing hormonal writer (scaled by `colony_primer_gain`). Any cluster can thus deposit a slow physiological nudge alongside a fast signal in one emission — the way real chemistry does — without gaining direct hormonal-write access.

**Recruitment, division of labor, and the mobilization cascade.** Colony defense scales with need: a detection lowers nearby responders' thresholds in proportion to need, low-threshold individuals act first, and higher-threshold reserves mobilize only as the need escalates — the response-threshold model of division of labor (Ramírez-Moreno et al., 2025). This is implemented as recruitment expressed as *just another channel in the chemical snapshot*, with no new firing logic: `bus.recruit(cluster, level)` raises a decaying per-cluster recruitment level; each cluster's once-per-turn snapshot injects it as a `RECRUIT` channel; recruitable switches declare `modulators={"RECRUIT": -coeff}`, so a recruitment level above neutral *lowers* their `effective_threshold` through the existing modulation math. A small settings-driven policy maps need signals already computed each turn — query difficulty/salience, the threat quorum, surprise/novelty, `requires_memory` — to `(target_cluster, gain)` pairs, so a hard turn recruits more frontal drafters, a memory-laden turn recruits deeper hippocampal recall, sustained threat recruits caution. Because recruitment level decays and `effective_threshold` is clamped, the positive-feedback loop is graded and self-limiting: as recruited responders satisfy the need, the need signal falls and recruitment relaxes. *Multi-need allocation:* when several needs compete, `allocate_recruitment` draws a Boltzmann (softmax) distribution over them at temperature `colony_recruit_softmax_temp` and shares a bounded recruitment budget across clusters rather than letting each mobilize independently — the maximum-entropy generalization of response thresholds to multiple task types (Lynch & Pavlic, 2024).

**Satisfaction and stop signals.** Recruitment that only rises and passively decays risks thrashing. *Start* (response) thresholds react fastest but cause the most task-switching, while a *stop* (satisfaction) threshold minimizes switching and a composite of the two best matches supply to demand (Lynch, Wilson, & Dornhaus, 2024). This layer adds an active `bus.satisfy(cluster, amount)` called when a recruited response succeeds — e.g. when the frontal cluster commits a draft whose critic score clears `colony_satisfy_critic_floor`, it satisfies `frontal` in proportion to the score, so recruitment drops on *success* rather than only on a slow timer. This converts the cascade from response-only to composite.

**Quorum-triggered commitment phase-shift.** In collective nest choice, once a quorum considers an option acceptable, recruitment shifts from slow deliberation to fast commitment (Chan et al., 2025). When the threat channel reaches quorum, the frontal cluster flips a transient commit mode — fewer drafters, tighter articulation timeout — the system moving from deliberation to decisive action under sustained threat, reusing the same quorum signal the slope-aware `quorum` produces.

**Sensory-filter specialization.** Division of labor needs an axis along which the personas genuinely differ. Response-*threshold* variation — the intuitive choice, and the one the bee defense literature first suggests — turns out to be the wrong axis: it can perform *worse than random* and does not improve task allocation unless the workers are already genuinely specialized (Lynch, Wilson, & Dornhaus, 2024), which thinly-differentiated drafters are not. The axis that works is *perceptual*: ant division of labor stems substantially from differing *ability to detect* task stimuli — odorant-receptor expression — rather than differing response thresholds (Caminer et al., 2023). Specialization is therefore implemented as a deterministic per-persona `sensory_gain` — a sensitivity vector over feature categories (affective, analytic/epistemic, novelty, threat) spanning ±`colony_sensory_gain_span` — that scales a category-tagged switch's input level before it evaluates firing. The Empath genuinely *detects* affective cues more readily, the Analyst epistemic ones: a real differentiation axis rather than injected noise.

**Live trail reinforcement (fast plasticity).** The wiring graph's edge weights are *read* live every turn (drafter selection, recall fan-out) but *written* only at sleep — so within a session the routing graph cannot adapt at all. Biology has both fast (eligibility/short-term potentiation, on use) and slow (consolidation) plasticity; the base system implemented only the slow half, and ant trails strengthen *as they are walked*, not in a nightly batch (Salman et al., 2024; Porras-Brenes & Amador-Vargas, 2024). This layer adds a transient, non-persisted *trail overlay* on the wiring graph: `get_edge_weight` returns `persisted_weight + trail_overlay` (clamped), so trails immediately bias the same live reads with no new read sites. At end of turn a path that fired *and paid off* (using the same `fired_path` eligibility trace and lightweight outcome the Hebbian updater uses) has its edges reinforced, biasing the *next* turns this session; the overlay decays at `colony_trail_half_life_s` and evaporates, while sleep still computes the durable update from traces. Because this is a genuine feedback loop, its reinforcement is held to an automated correlation check — trail deltas must concentrate on edges from high-outcome turns, not low-outcome ones — and its behavioral effect is measured directly: under fixed wiring, identical probes route differently with the overlay active than without it, above the within-condition noise floor (§6.9).

**Aggregate-state neuromodulation feedback.** Finally, the colony's overall activity feeds back into its chemistry. Neuromod deltas otherwise come only from appraisal of the *input*; this layer adds a small, clamped term driven by the system's aggregate processing state on the **prior** turn — firing volume → arousal/Glu, convergence vs. conflict → DA or GABA, inhibition load → GABA — applied by the hypothalamus, the single chemistry writer. Reading the prior turn's aggregate (not the current one), at small gain, with the contribution clamped and logged, bounds the activity → chemistry → processing loop by construction. The existing fast→slow chemistry coupling then integrates it over time. This makes the chemistry partly self-referential: it reflects not only what the user said, but what the system itself was doing (effortful, cohered, conflicted).

This is the architecture's most explicitly emergent layer — quorum, silence, recruitment, and mobilization are colony-level behaviors that no single cluster decides, the same way criticality (§4.5.1) is a network-level property no single switch sets. The genuine feedback loops it introduces (concentration, recruitment, chemistry self-feedback, and live trail reinforcement) are each individually instrumented in the decision log and built for stability by construction: small gains, hard clamps, and reads of the *prior* turn's aggregate rather than the current one, so the activity → chemistry → processing loop is bounded.
...
A **plasticity modulator** scales the learning rate by session-averaged DA × ACh — engaged, high-DA sessions learn faster; flat or disengaged sessions learn slowly. A gentle homeostatic decay (1% toward resting weight 1.0 per update) prevents lock-in. Edge weights are consulted live by three learning surfaces, each with a *consume* mechanism matched to what it routes: **drafter selection** samples drafters in proportion to a softmax over their learned weights, so a shift in the Hebbian ranking changes the response *mix*, not merely the top pick; **switch evaluation order** converts a switch's learned weight into a bounded, direction-aware *synaptic efficacy* that scales its firing threshold (a stronger route fires more readily), with per-switch bands so a safety gate can never be learned past its allowed direction; and **recall fan-out** uses the learned weights to set the schema-versus-episode division of a fixed recall budget. This is reinforcement expressed through routing rather than an explicit RL objective.

**Graded per-turn plasticity.** The session-averaged modulator above is a *session-level* three-factor rule (Frémaux & Gerstner, 2016; Gerstner et al., 2018), but on its own it is too coarse: neuromodulation continuously scales — and can even flip the sign of — plasticity rather than switching it on or off (Ang et al., 2021), and aversive arousal follows an *inverted-U*, not an off-switch — moderate arousal *facilitates* consolidation and emotionally intense events (including aversive ones) imprint *more* strongly; only extreme stress impairs (Lukšys & Sandi, 2011; Kim et al., 2015; Rezayof et al., 2022). Plasticity is therefore graded at turn granularity by a per-turn factor keyed primarily to **arousal/emotional intensity**, not valence sign: ACh, NE, surprise/salience, and the DA swing raise it, and *intensity* `|valence|` raises it for *either* sign, so an intense aversive turn imprints hard (the fear-learning case) rather than being skipped. Only very high CORT/GABA pulls it down — the descending limb of the inverted-U knee — a graded high-stress dampener rather than an all-or-nothing gate; the signal-quality hard skips (near-zero outcome, dissociated emotion) are retained. The per-turn factor multiplies the path delta, mapped into `[plasticity_turn_min, plasticity_turn_max]`, and is reported per turn in the decisions log. This is the rule §4.5.1 refers to when it keys learning to state *level* while keying default-mode gating to state *velocity*.
...
**Closing the loop on all three surfaces.** Each surface is *written* as well as read, so a learned weight both changes behavior and is updated by its own outcome. Two of the three edge families sit off the literal firing path — a switch's `sensory.text→temporal.<switch>` edge and the `mem.recall→hippocampus.<strategy>` recall edges are never consecutive nodes on `fired_path` (the source is a bus channel or a budget allocation, not a fired node), so the path-level Hebbian update cannot reach them. They are therefore credited explicitly, mirroring competitive drafter reinforcement: **switch-routing credit** reinforces the edge of each content-gated switch that fired on a good-outcome turn, and **recall-routing credit** reinforces the recall pathway (schema or episode side) that actually surfaced memory that turn, in proportion to its contribution. Both run at consolidation under the same composite outcome and graded plasticity, are half-scaled relative to the path-level update to keep them gentle, and are individually flag-gated. All three Hebbian learning surfaces named in the architecture — drafter selection, switch evaluation order, and recall fan-out — are thus live end to end: a learned weight both biases behavior and is reinforced by its outcome.

### 4.8 Dynamic reasoning framework selection

A major architectural addition is a per-turn skill routing system that dynamically selects a reasoning or emotional-intelligence framework from a self-reflection library of roughly 380 skills (`brain/skills/`) and injects it into the response drafters' prompts. The selection is invisible to the user — it shapes how the drafters think, not what they explicitly say — and is designed to be used as a "habit of thought" rather than a script. One implementation nuance matters for honesty: skill text is injected only into **local-model (Ollama) prompts**; cloud/Claude integrators ignore the injected block and rely on the model's own capability. The *selection* still runs and is logged on every qualifying turn regardless of backend, so the routing trace is complete even when the chosen frontend model does not consume the text. The motor cortex draws on a separate **general-purpose task-skill library** (~170 entries) invoked natively by its Claude-backed planner (§4.9), giving two distinct skill populations: self-reflection frameworks for the conversational/DMN/rumination paths, and task skills for autonomous execution.

**The skill library** spans 27 categories: logic, ethics, creativity, strategy, writing, decision-making, probability, game theory, epistemology, psychology, systems thinking, narrative, communication, emotional intelligence, and more. Each category contains a router (overview level) and multiple leaves (specialized techniques). A 768-dimensional embedding index (nomic-embed-text) is precomputed across all entries and loaded at boot. Four tier-1 baseline skills — logic check, communication clarity, ethics bias check, and emotional awareness — are injected into every qualifying turn as a permanent baseline; the remainder form a ranked pool selected per turn.

**Selection operates on three paths**:

- **Conversational path** (user-facing turns): First gated — chitchat, simple acknowledgements, and low-stakes turns skip selection entirely. Substantive turns and any turn where the user is in emotional distress activate selection. The current input plus executive key points are embedded and ranked against the full library by cosine similarity. If the top candidate's score exceeds the second by a clear margin, it is selected directly without an LLM call. Otherwise, the top five candidates are forwarded to a lightweight LLM (Haiku) for disambiguation, which returns a skill selection and a brief rationale. The selected skill's block is then injected into each drafter's prompt.
...
The system is fully operational with all major clusters implemented. The codebase as of June 2026 includes:
...
- Dynamic reasoning-framework selector (`skill_loader.py`) with a ~380-entry embedding-indexed self-reflection skill library (injected into local-model prompts), sticky-context reuse, and conversational / autonomous-DMN / rumination selection paths; plus a separate ~170-entry general-purpose task-skill library invoked natively by the Claude-backed motor planner
- Stigmergic coordination layer (`bus.py`, `clusters/`, `wiring.py`, `dmn.py`, `neuron.py`): topic concentration with slope-aware quorum and silence-as-signal (an UNARMED/ARMED/QUIET state machine), releaser+primer messages drained through the hypothalamus, recruitment-based mobilization with softmax multi-need allocation and an active satisfaction/stop signal, quorum→commit phase-shift, per-persona sensory-filter specialization, live trail reinforcement on the wiring graph, and prior-turn aggregate-state neuromodulation feedback
- Criticality layer (`observability/criticality.py`): branching-ratio σ estimator with arousal-modulated setpoint feedback control of the global modulation gain, plus per-turn chemistry trajectory feeding default-mode gating
- Graded per-turn Hebbian plasticity (three-factor + inverted-U) replacing the legacy binary skip
...
- 1,334 pytest tests across 70 test modules — including the three learning surfaces' credit paths and bounded-efficacy safety bands, the coordination layer's silence state-machine transitions, slope-aware quorum, and self-limiting-feedback proofs
- Controlled learning-divergence harness (`eval/wiring_divergence_ab.py`): trains two experiential regimes, probes them on identical inputs with memory wiped and chemistry pinned, and measures routing divergence against a frozen-wiring causal control, a within-regime noise floor, a label-permutation test, and a bootstrap confidence interval
- Embedding-based intent detector (`intent_detector.py`) backing the temporal `self_reference` / `epistemic_action` gates: a per-persona exemplar bank matched by cosine, seeded from canonical phrasings and grown when the understanding integrator confirms an intent the match missed — keeping intent detection paraphrase-robust and off the LLM critical path
...
The eval dataset consists of **393 turns across 69 sessions**, logged in `eval/turns.jsonl` with 1,320 associated decision records. Sessions average 5.7 turns each. The decision log now also carries skill-router records (`skill_selector_pick`, `skill_selector_gated_out`, `skill_active_reused`, `dmn_skill_pick`), giving the first runtime trace of the dynamic reasoning-framework selector described in §4.8. This is a growing dataset at an early stage; all findings should be read as directional indicators rather than statistically robust conclusions.

### 6.2 LLM call efficiency

Mean LLM calls per turn: **3.38** (range 0–11). This sits within the designed range of 3–5 for typical turns; the self-directed task system and the skill-disambiguation step lift the mean on the turns where they fire. The 0 minimum confirms the template-match switch path is functional. The maximum of 11 exceeds the designed ceiling of 8 on a small number of autonomous task-completion turns, where the strategic_planner, follow_through loop, and ResultReporter cells stack additional integrator calls on top of the normal drafting pipeline.

Mean latency: **13.1 seconds** (range 0.8–83.1s). The distribution is bimodal: short conversational turns sit in the 1–3s range, while autonomous multi-step task turns (planner plus a step-by-step follow-through loop) and the skill-selection disambiguation step are the longest, reaching the 83.1s maximum and pulling the mean up. Latency is the primary user-facing limitation; it is framed in the design as "deliberation time" rather than lag, with the real-time brain activation visualization serving as evidence of live processing rather than a loading state.

### 6.3 Predict-and-surprise gating

**100 of 393 turns (25.4%)** produced actual integrator suppression with measurable LLM call savings. An additional 35 turns triggered the gating predictor but were overridden by the emotion-aware veto before suppression could occur, for a total of 135 candidate gating events (34.4% of turns).
...
The 25.4% save rate is consistent with sessions averaging 5.7 turns — within-session predictor history is thin at that length, which limits how far the rate can climb per session. Longer sessions are the primary lever for approaching the 30–50% design target.

### 6.4 Neuromodulator and hormonal dynamics

The five fast neuromodulator levels show expected biological-analog behavior across the dataset:

- **DA**: mean 0.396, range 0.300–0.850. Active reward signaling with a clear 0.300 floor and headroom for high states.
- **ACh**: mean 0.545, range 0.129–0.850. Consistently higher than DA, reflecting primarily exploratory and self-referential session content.
- **GABA**: mean 0.072, range 0.020–0.850. Low resting inhibitory tone; the 0.850 maximum confirms the hostile/threat pathway is functional.
- **Glu**: mean 0.286, range 0.150–0.502. Moderate arousal, tracking input rate and urgency.
- **NE**: mean 0.433, range 0.197–0.756, present on the 139 most recent turns (the channel was added partway through the dataset). The values sit largely in the 0.20–0.55 optimal band of the inverted-U; the 0.756 maximum reaches into the over-activation range associated with the `scattered` overlay.

**The slow hormonal tier produces logged values over the same 139-turn window** (the hormones, like NE, were added partway through the dataset): serotonin (5HT) mean 0.486 (range 0.429–0.530), cortisol (CORT) mean 0.039 (range 0.000–0.050), oxytocin (OXT) mean 0.320 (range 0.294–0.420), anandamide (AEA) mean 0.313 (range 0.219–0.530). The pattern is internally coherent: serotonin holds a stable affective baseline near 0.49; cortisol rests near its 0.0 floor across non-hostile sessions, with the 0.05 ceiling confirming the stress-accumulation pathway is wired but rarely engaged; oxytocin has begun to climb above its 0.3 resting level (max 0.42), consistent with the slow trust-building dynamic that drives the `connected` and `warm` states; and anandamide tracks arousal, rising toward 0.53 on the highest-NE/Glu turns where its homeostatic-buffer role engages. These are baselines from a benign environment rather than evidence of the cross-channel antagonisms (OXT↓CORT, AEA↓NE) firing — those require adversarial or high-arousal sessions not yet in the dataset.

ACh declines across the dataset: the earliest-quintile mean is 0.444 and the latest-quintile mean is 0.409. DA declines in parallel (early 0.410 → late 0.344), as do GABA (0.080 → 0.030) and Glu (0.339 → 0.232). All four fast channels moving in the same direction rules out a simple measurement artifact and points to a broad settling of arousal as the environment becomes familiar.

The per-session dominant-emotion trajectory across the 69 sessions, reconstructed from turn-level state, runs: an extended `neutral` plateau through roughly the first 28 sessions; a `curious`/high-ACh exploratory peak (sessions 29–37, where ACh reached 0.75–0.81 in the longest sessions); a diversification phase into `confident`, `thoughtful`, `joy`, and `lively`; and most recently an oscillation between DA-floor `scattered` sessions (DA pinned at its 0.30 floor) and recovered states (`settled`, `excitement`, `engaged`, `content`), with the most recent session at `excitement`. The DA-floor/low-arousal `scattered` sessions are a distinct regime: the system attending to inputs without positive reinforcement. Two independent signals — turn-level neuromod state and the session-end self-model synthesis written to `self.md` — describe the same trajectory.

A plausible interpretation: familiarity is accumulating. ACh encodes novelty; if sessions increasingly revisit familiar conversational patterns over 69 sessions of history, the novelty signal should attenuate. This is the expected long-run trajectory for a system learning its environment. The mid-dataset ACh peak (driven by several unusually long exploratory sessions) and the persistent topic-uniformity confound have not been controlled and remain live alternative explanations.
...
Emotion distribution observed across all 393 turns: **curious (31%)**, neutral (30%), scattered (10%), thoughtful (9%), excitement (5%), confident (4%), alert-curious (4%), lively (2%), wistful (1%), engaged (1%), joy (1%), serene (1%), settled (1%), content (1%), with a single occurrence of `somber` (<1%). Fifteen distinct states are observed. The appraisal-based overrides (embarrassed, sympathetic, flirty, etc.) have not been triggered in this dataset — sessions have been non-hostile and non-emotionally-charged — but the infrastructure is confirmed functional.

The most notable entry is **`somber`, the only negative-valence base state in the dataset**. Its single occurrence is too thin to interpret, but its appearance confirms that the low-valence region of the emotion lookup table is reachable from organic session dynamics, not only from the hostile/threat pathway that drives GABA — every other observed base state is neutral or positive.

`scattered` at 10% remains the third most common state. It maps to a recognizable neuromod signature: elevated NE (above the scatter threshold) combined with reduced ACh and DA near its floor — an over-activated attentional system with insufficient reward signal, a kind of internally fragmented attention. This state is an NE overlay, not a base chemistry label: it emerges when the NE color-overlay rule fires on top of a low-DA, low-reward base state, and the now-logged NE values (§6.4) make that mechanism directly visible in the data. It correlates with the DA-floor sessions in the trajectory above. The non-canonical states collectively account for 39% of turns, reflecting a system whose emotional range spans more of its state space than the neutral/curious core.
...
The dataset now includes three sleep consolidation passes. The first two ran under the earlier outcome formula; the third is the first to run under the refined signal described in §4.7:

- **Plasticity modulator**: 1.17, 1.14, and 1.08 (all above 1.0 — DA×ACh product elevated learning rate, indicating engaged sessions)
- **Edges updated per session**: 3, 3, and 13
...
- **Pass 3 top gainers**: `temporal.understanding_integrator → frontal.executive`, `frontal.drafter_A → frontal.commitment_extractor` (new edge type), `temporal.length_bucket → temporal.understanding_integrator`
- **Top losers**: none in any pass

The `temporal.understanding_integrator → frontal.executive` edge was reinforced in all three passes — the single most consistently strengthened connection in the system. `frontal.executive → frontal.drafter_A` was reinforced in the first two passes. The third pass, the first under the refined formula, also logs a `signal_quality` summary (1 turn with a critic score, 5 with detected user emotion, 5 with a usable DA delta out of 5 scored turns), and it updated far more edges (13) than the earlier passes — the lowered skip threshold (0.02, down from 0.05) is letting more turns contribute. A new lead edge, `frontal.drafter_A → frontal.commitment_extractor`, also appears, suggesting the system is reinforcing the path from its preferred drafter into the commitment-extraction step. Weight deltas remain small (0.004–0.007 per session).

Notably, **no edge weakening appears in the production passes even under the refined formula**. The refined signal is built to produce negative deltas — turns where DA drops during the turn penalize their traversed edges regardless of session-level mood — but the benign sessions scored under it contain few such turns. Whether long-term depression materializes at scale requires more sessions and a wider range of input topics than these topically-uniform, non-hostile conversations provide.

The distinct and more fundamental question — whether the reinforcement *changes how the system processes input at all*, rather than merely accumulating weight — is not answerable from observational production data, where learned weights, memory content, and chemistry are confounded. It is addressed directly by the controlled experiment in §6.9, which holds memory and chemistry constant and isolates the learned weights as the only variable.

### 6.8 Draft quality

Mean drafter count: **1.12 across 439 drafts**. The single-draft norm confirms that arousal-modulated drafter count selection is working correctly: most turns are low enough arousal to generate one draft, with multiples reserved for complex or emotionally charged inputs. Critic scores are captured in `TurnTrace.draft_scores` for multi-draft turns (where the LLM critic ran); single-draft turns carry `critic_ran=False` and do not contribute a critic term to the Hebbian outcome, preventing a spurious positive bias from a hardcoded fallback.
...
### 6.9 Controlled test: does learned wiring change how the brain routes?

The observational data above cannot answer the architecture's load-bearing question — whether experience *changes how the system processes input* — because in production the learned weights are confounded with memory content, chemistry, and topic. A controlled experiment removes those confounds. Two copies of the system are given divergent experiential histories: a **warm** regime trained on affectionate, relational turns and an **analytical** regime trained on precise, technical ones, each consolidated over ten sessions from an identical starting circuit. Both are then **probed on the same inputs with episodic and schema memory wiped empty and chemistry pinned to a neutral baseline**, so the only thing that can differ between them is the learned edge weights. The probe behavior is compared three ways: warm-versus-analytical (the effect), the same comparison with weighted routing disabled (`BRAIN_WIRING_FROZEN`, the causal control), and repeat-versus-repeat within a single regime (the noise floor). Significance is a label-permutation test; the primary effect carries a bootstrap confidence interval. The learning rate is amplified ×5 during training to compress what would otherwise take roughly five times as many sessions into a tractable run; the natural-rate caveat is carried throughout.

**The causal channel is confirmed.** On the aggregate firing-path metric (Jaccard distance over the set of switches and integrators that fire on each probe), warm and analytical diverge by 0.117 (95% CI [0.084, 0.152]) against a frozen-control divergence of 0.066 and a permutation p = 0.002. The divergence depends on the weights being read: freezing the wiring nearly halves it. What the system does with an identical input is shaped by what it has learned, not only by its inputs and chemistry.

**The recall fan-out surface shows this most cleanly.** The learned schema-versus-episode recall split diverges 0.110 between the two regimes against 0.012 for both the frozen control and the noise floor — an order-of-magnitude separation (p = 0.002). The mechanism is visible in the weights: the analytical regime drives the episodic-recall edges (`cosine_recall`, `time_filter`) to ≈1.56 while the warm regime leaves them near baseline, so the two instances allocate a fixed recall budget differently when asked the same question. This is the clearest single piece of evidence that experience changes processing, and it comes from the most recently completed of the three learning surfaces.

**The drafter surface diverges in weight but only modestly in expression.** The learned drafter preference reorders almost completely between regimes (Kendall-τ 0.90 between the two weight rankings — the warm regime favors drafters A and E, the analytical regime favors C and demotes A), yet the realized selection distribution diverges only 0.055 (frozen 0.021). The reason is mechanical: the reordered weights all still sit near 1.0, and softmax sampling over near-equal weights produces near-equal selection. Structural learning is real and substantial; its behavioral footprint grows as the weights separate further with more history.

**The switch-evaluation-order surface shows no differentiation.** The three content-gated switches (`template_match`, `self_reference`, `epistemic_action`) remain at their initial weights in both regimes, and their firing rates do not diverge. These switches are gated on input content that the two training corpora did not differentiate, so neither regime accrued differential credit on them. This is an honest negative: the surface is wired and credited, but this experiment did not exercise it.

**The network-aggregate effect is real but small.** The 0.117 aggregate divergence sits essentially at the within-regime noise floor (0.116): the permutation test establishes that the *specific* warm-versus-analytical contrast is non-random, but the whole-network behavioral footprint at this scale and history is comparable to run-to-run variation. The signal is concentrated in the recall surface rather than spread across the network.

**Live trails produce a separable within-session effect.** Holding the wiring fixed, identical probes route differently with the live trail overlay (§4.5.2) active than without it — firing-path divergence 0.202 against a within-condition noise floor of 0.142 (p = 0.002). The fast, transient plasticity layer changes routing within a session, distinct from and larger than the slow consolidated effect.

**What this establishes and what it does not.** It establishes, under a clean causal control, that the learned weights are a real channel through which experience changes processing — the architecture's central premise is not merely implemented but demonstrably load-bearing — and that the recall surface and the live-trail layer carry that effect most strongly. It does not establish that the effect is large at the natural learning rate (the rate was amplified ×5), that it generalizes beyond this synthetic probe set, or that it improves response quality; the samples are small (ten sessions, five probe repeats) and the probes are constructed rather than drawn from production. It is evidence that the mechanism works as designed, not yet that it matters at scale.
...
**The cost model is validated.** Mean 3.38 LLM calls per turn is within the designed range. The system is operating at roughly the expected budget, with the switch-only fast paths (0 LLM calls for trivial inputs) confirmed functional and the elevated calls concentrated in autonomous task and skill-selection turns rather than routine handling.

**Predict-and-surprise gating is stable and producing consistent savings.** 25.4% of turns are successfully gated, consistent with session-length constraints rather than mechanism failure. The `switch_suppressed_by_modulation` decision type (30 occurrences) shows the neuromodulator system influencing computation at the individual switch level, not only at the integrator-suppression level. This is the most granular evidence that chemistry is doing real computational work.

**The neuromodulator and hormonal dynamics are functional and showing longitudinal behavior.** All four core fast channels decline from early to late sessions (ACh 0.444 → 0.409, DA 0.410 → 0.344, GABA 0.080 → 0.030, Glu 0.339 → 0.232), a broad settling of arousal as the environment becomes familiar. The per-session trajectory runs neutral plateau → curious/high-ACh exploratory peak → diversification into confident/thoughtful/joy/lively → recent oscillation between DA-floor scattered and recovered states, with two independent signals (turn-level state and session-end self-model) in agreement. The slow hormonal tier (5HT, CORT, OXT, AEA) and NE produce coherent logged values — cortisol at its floor, oxytocin accumulating, serotonin holding a stable baseline. The cross-session ACh decline is notable: if it reflects genuine familiarity accumulation rather than topic-content bias, it is evidence of long-horizon environmental adaptation in the neuromodulator system.

**Memory recall is active.** Contributing to 35.9% of turns means the hippocampus is a live participant in the pipeline, not merely an accumulating store.

**Learned wiring causally changes routing.** The controlled divergence experiment (§6.9) is the strongest result in the dataset: two instances given different experiential histories route identical inputs differently, the difference collapses under a frozen-wiring control (p = 0.002), and the effect is cleanest on the recall surface (≈9× over control). The architecture's central premise — that experience reshapes processing through learned weights — is not merely implemented but demonstrably load-bearing, with the important qualifier that the effect is concentrated in one surface and modest in the network aggregate at this scale.
...
**Predict-and-surprise reaching its full efficiency target.** The 25.4% save rate is consistent with session-length constraints — at 5.7 turns average, the within-session predictor history is too thin to push significantly higher. Longer sessions remain the primary lever. The saves that do occur are at 1.00 confidence and are structurally correct.

**Hebbian plasticity producing emergent behavioral style at the natural rate.** The controlled experiment (§6.9) confirms the mechanism under amplified training: learned weights diverge by experience and change routing, most clearly on the recall surface. What remains unevidenced is whether this accrues at the *natural* learning rate over real sessions. The production passes show consistent reinforcement of the same core pathway (`temporal.understanding_integrator → frontal.executive` strengthened across all three consolidation passes), with plasticity modulators correctly elevated on engaged sessions, but the deltas are small and no long-term depression has appeared in the benign production sessions. Whether preferred drafters, recall paths, and switch orderings emerge from organic reinforcement — and whether the divergence seen under ×5 amplification reproduces at 1× over many more sessions — is the months-long experiment.

**The emotion model producing differentiated behavior.** Fifteen distinct states are observed; `scattered` at 10% is the most notable recurring signal, correlating with the DA-floor sessions, and `somber` is the sole negative-valence base state. Deliberate emotional expression (§4.9) adds a further dimension: the system can perform an emotional register for communicative effect without modifying its chemical state. Whether either authentic emotion or deliberate expression produces measurably different response quality on matched inputs requires controlled comparison not yet conducted.
...
**The stigmergic-coordination and criticality layers.** These are the architecture's strongest bets on emergence: their predicted behaviors are collective and not decided by any single cluster — quorum responses to sustained signal, silence-triggered recall, need-proportional mobilization, and a network held near its maximally-responsive branching ratio. The open question is not whether the mechanisms run — the tests confirm they do — but whether they produce measurably more coherent long-window behavior than the neuromodulator layer alone, and whether their feedback loops stay stable over very long sessions. Each loop is individually instrumented in the decision log so that this can be measured directly. The reason to expect a payoff is the same reason the mechanisms were adopted: they are convergent natural solutions to decentralized coordination, observed independently in neural tissue, insect colonies, and flocks, which is a strong prior that they are doing real work rather than decorating the architecture. That prior still needs to be confirmed in this system's own longitudinal data, which is the next-phase evaluation.
...
**Whether organic Hebbian reinforcement reflects preference or topic artifact.** In production, the drafter_A reinforcement could reflect a learned stylistic preference or simply mirror the dataset's topic uniformity, and the observational data cannot separate the two. The controlled experiment (§6.9) removes this confound for the routing question by construction — divergent regimes probed on identical inputs — and finds genuine learned divergence. What it does not settle is whether the *production* reinforcement, accrued from topically-uniform sessions at the natural rate, is preference or mirror; that needs a contrast class of organically diverse sessions.
...
**Latency.** Mean 13.1 seconds is workable but not conversational. The cost is concentrated in new capability rather than routine handling: autonomous multi-step task turns and the skill-disambiguation step are the longest turns, reaching an 83.1s maximum. The system was designed with the expectation that latency would be a primary UX limitation, framed as "deliberation" rather than lag. Shorter conversational turns achieve sub-second to 2-second latency; complex multi-drafter and autonomous-task turns drive the mean up. This is a fundamental constraint of sequential LLM calls on the critical path.
...
**Extended Mind**: The second brain satisfies Clark and Chalmers' coupling and availability conditions: it is reliably available, automatically endorsed, and directly accessible. The entity's responses are demonstrably shaped by second-brain content on 35.9% of turns. The functional loop is closed.

---

## 10. Conclusions

This system is a genuine implementation of biologically-inspired cognitive architecture at a scale and fidelity not previously published in the LLM multi-agent literature. Its core design claims — sparse LLM activation at convergence zones only, neuromodulator dynamics as free persistent state, hippocampus-gated memory with a vector episodic store, Hebbian plasticity in edge weights, predict-and-surprise gating from Active Inference — are not merely described but implemented and operational.

The strongest result is the one the architecture is built to produce: **experience changes how the system processes input, through learned weights, under a clean causal control.** The divergence experiment (§6.9) shows two instances given different histories routing identical inputs differently, the effect collapsing when the wiring is frozen (p = 0.002), cleanest on the recall surface (≈9× over the frozen control) and accompanied by a separable within-session trail effect. The premise is load-bearing, not decorative — with the honest qualifiers that the effect is concentrated in one of the three learning surfaces, modest in the network aggregate, and measured under a ×5-amplified training rate on a synthetic probe set.

The 393-turn, 69-session operational dataset supports the following:

1. **The cost model holds.** Mean 3.38 LLM calls/turn is within the 3–5 design range; the 0-call floor and the 11-call ceiling on autonomous task turns are both as designed.
2. **Predict-and-surprise gating is stable** at 25.4% of turns; the rate reflects session-length constraints rather than mechanism failure, and `switch_suppressed_by_modulation` (30 occurrences) shows chemistry gating computation at the individual-switch level.
3. **The full nine-channel chemical system runs and is internally coherent.** The fast neuromodulators settle across sessions (ACh 0.444 → 0.409, DA 0.410 → 0.344, GABA and Glu in parallel); the slow hormones hold sensible baselines (cortisol at its floor, oxytocin accumulating, serotonin stable); NE sits mostly in its optimal band. If the cross-session settling reflects familiarity rather than topic bias, it is evidence of long-horizon environmental adaptation.
4. **All three Hebbian learning surfaces are closed-loop and causally active.** Drafter selection, switch evaluation order, and recall fan-out each bias live computation and are reinforced by outcome; the controlled experiment (§6.9) confirms learned weights change routing, and consolidation reinforces a consistent core pathway across passes.
5. **Memory recall is active** on 35.9% of turns, with a `reuse_recent_recall` efficiency path visible in the data.
6. **The emotion model expresses 15 distinct states**, including a single negative-valence base state (`somber`); `scattered` (10%) tracks the DA-floor sessions.
7. **The capability surface is broad and operational**: self-directed task system, REM-style DMN thought consolidation, inner-monologue integration, deliberate emotional expression, autonomous DMN with urgency-ranked deferred thoughts under a background budget, and a per-turn reasoning-framework selector over a ~380-skill self-reflection library.
8. **The coordination layer adds two convergent natural designs** beyond the brain's chemistry — stigmergic coordination and division of labor from social-insect colonies (quorum/silence, need-proportional recruitment, sensory-filter specialization, live trail reinforcement, aggregate-state chemistry feedback), and branching-ratio criticality control with arousal as its parameter from flock dynamics — adopted because patterns that evolve independently in brains, colonies, and flocks are strong evidence of fundamental decentralized-coordination designs. Their emergent payoff is the subject of next-phase evaluation.
9. **The observability infrastructure is complete**, with 1,334 tests across 70 modules, and is what makes the analysis above possible.

What requires more data: whether organic reinforcement at the natural rate produces the behavioral divergence seen under amplification, long-term depression in adversarial sessions, empathy-critic and appraisal effects in emotionally diverse sessions, and long-horizon retrieval quality.

The central comparative question — whether the multi-agent architecture produces better *responses* than a single well-prompted LLM at equal compute — remains untested, and the honest prior from the literature is that structure buys legibility and persistent state more than raw quality. This architecture's distinct claim is not raw quality but longitudinal difference: that a system organized around affect as a real, load-bearing state develops differently over time. The divergence experiment is the first controlled evidence that the substrate for that difference — experience reshaping processing through learned weights — is real, concentrated for now in the recall pathway and modest in aggregate.

Framed against the field's prevailing ambition, the contribution is best read not as a step toward artificial *general* intelligence but as a working prototype of artificial *emotional* intelligence (AEI): a system whose intelligence is organized around affect as a persistent, computationally load-bearing state rather than around breadth of task capability. The substrate is active across all designed mechanisms, and experience demonstrably reshapes it.
...
Ang, G. W. Y., Tang, C. S., & Hay, Y. A. (2021). The functional role of sequentially neuromodulated synaptic plasticity in behavioural learning. *PLOS Computational Biology*, 17(6), e1009017. https://doi.org/10.1371/journal.pcbi.1009017
...
Baltiansky, L., Frankel, G., & Feinerman, O. (2023). Emergent regulation of ant foraging frequency through a computationally inexpensive forager movement rule. *eLife*, 12, e77659. https://doi.org/10.7554/elife.77659
...
Caminer, M. A., Libbrecht, R., & Majoe, M. (2023). Task-specific odorant receptor expression in worker antennae indicates that sensory filters regulate division of labor in ants. *Communications Biology*, 6(1). https://doi.org/10.1038/s42003-023-05273-4

Carroll, M. J., Brown, N., & Huang, E. (2025). E-β-ocimene and brood cannibalism: Interplay between a honey bee larval pheromone and brood regulation in summer dearth colonies. *PLOS ONE*, 20(2), e0317668. https://doi.org/10.1371/journal.pone.0317668
...
Chan, S., Hemingway, C. T., & Weinberg, I. P. (2025). *Nest attributes influence choice accuracy, but not decision speed in acorn ants* [Preprint]. https://doi.org/10.1101/2025.07.20.665794
...
Giurfa, M. (2025). The cognitive side of communication in social insects. *Trends in Cognitive Sciences*, 29(11), 979–981. https://doi.org/10.1016/j.tics.2025.08.005

Gryboś, A., Skowronek, P., & Bryś, M. S. (2025). The pheromone landscape of *Apis mellifera*. *Molecules*, 30(11), 2369. https://doi.org/10.3390/molecules30112369
...
Kim, E. J., Pellman, B. A., & Kim, J. J. (2015). Stress effects on the hippocampus: A critical review. *Learning & Memory*, 22(9), 411–416. https://doi.org/10.1101/lm.037291.114

Lazarus, R. S. (1991). Emotion and adaptation. Oxford University Press.

Lukšys, G., & Sandi, C. (2011). Neural mechanisms and computations underlying stress effects on learning and memory. *Current Opinion in Neurobiology*, 21(3), 502–508. https://doi.org/10.1016/j.conb.2011.03.003

Lynch, C. M., & Pavlic, T. P. (2024). *Generalizing the response and satisfaction threshold models for multiple task types: A maximal entropy approach* [Preprint]. https://doi.org/10.1101/2024.04.26.591219

Lynch, C. M., Wilson, R., & Dornhaus, A. (2024). *Stop and go: exploring alternative mechanisms for task allocation in social insects* [Preprint]. https://doi.org/10.1101/2024.05.13.593812

Minsky, M. (1986). *The Society of Mind*. Simon & Schuster.

Mondet, F., Beslay, D., & Cousin, M. (2025). Impact of temporal and rearing environment variations on brood ester pheromone production in *Apis mellifera*. *Comptes Rendus Chimie*, 26(S2), 119–128. https://doi.org/10.5802/crchim.270
...
Porras-Brenes, K., & Amador-Vargas, S. (2024). *Carrying oversized loads may create pheromone "blind spots" in leafcutter ants* [Preprint]. https://doi.org/10.1101/2024.12.11.628008
...
Ramírez-Moreno, D. M., Galizia, C. G., & Nouvian, M. (2025). Division of labour during honeybee colony defence. *Philosophical Transactions of the Royal Society B*, 380(1922). https://doi.org/10.1098/rstb.2023.0272

Rezayof, A., Sardari, M., & Hashemizadeh, S. (2022). Cellular and molecular mechanisms of stress-induced memory impairment. *Exploration of Neuroscience*, 1, 100–119. https://doi.org/10.37349/en.2022.00008

Rosenthal, D. (1997). A theory of consciousness. In Block, N., Flanagan, O., & Güzeldere, G. (Eds.), *The Nature of Consciousness*. MIT Press.

Salman, M., Garzón Ramos, D., & Birattari, M. (2024). Automatic design of stigmergy-based behaviours for robot swarms. *Communications Engineering*, 3(1). https://doi.org/10.1038/s44172-024-00175-7
...
*Data sources: `eval/turns.jsonl` (393 turns, 69 sessions); controlled learning-divergence run `eval/wiring_divergence_full_results.json` (§6.9), as of 2026-06-07*  
