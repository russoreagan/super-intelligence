...
We describe the design, implementation, and early empirical observations of a biologically-inspired cognitive architecture whose central hypothesis is that implementing the affective substrate of cognition — persistent emotional state that modulates computation rather than merely framing output — produces a system that develops differently over time. We frame the goal as **artificial emotional intelligence (AEI)** rather than artificial general intelligence (AGI): the design optimizes for affect as a load-bearing internal state that gates computation, not for breadth of task capability. The architecture maps explicitly onto regions of the human brain, with a two-tier chemical signaling system — five fast neuromodulators (dopamine, acetylcholine, GABA, glutamate, norepinephrine) and four slow hormones (serotonin, cortisol, oxytocin, anandamide) — coordinating a fixed set of heterogeneous, region-specific agents rather than a central planner. Language model calls are treated as expensive convergence-zone events, with the majority of per-turn computation delegated to cheap, deterministic switch neurons. The architecture draws from predictive processing theory (Friston, Clark), dual-process theory (Kahneman), Global Workspace Theory (Baars, Dehaene), Dennett's Multiple Drafts model, and Clark and Chalmers' extended mind hypothesis. The current dataset covers 393 turns across 69 sessions. The system operates within its designed cost envelope (mean 3.38 LLM calls/turn against a target of 3–5), demonstrates functional neuromodulator dynamics, and produces measurable predict-and-surprise gating saves (25.4% of turns). Memory recall is active on 35.9% of turns. Three Hebbian sleep consolidation passes — the most recent the first to run under the refined outcome signal — provide early evidence of edge weight reinforcement; the three Hebbian routing surfaces (drafter selection, switch evaluation order, and recall fan-out) are now each closed-loop, both biasing live computation and being reinforced by turn outcome rather than read-only. Fifteen distinct emotional states have been observed; `scattered` is the third most common at 10% and correlates with a recognizable elevated-NE, DA-floor neuromodulator signature, and `somber` has appeared as the first negative-valence base state. This is the first snapshot in which the slow hormonal tier produces logged values: cortisol rests near its floor, oxytocin has begun to accumulate, and serotonin holds a stable affective baseline. ACh declines across the dataset from early to late sessions — a longitudinal signal whose most plausible interpretation is that 69 sessions of history has made the system's conversational environment progressively more familiar. The system also incorporates deliberate emotional expression (audio/visual-only performance, decoupled from neuromodulator state), DMN autonomy with urgency-ranked deferred thoughts, a background resource policy governing autonomous computation, a self-directed multi-step task system, and a dynamic per-turn reasoning-framework selector drawing on a self-reflection library of roughly 380 skills. Beyond the brain's own chemistry, the coordination layer incorporates two further mechanisms that nature has converged on independently for decentralized coordination — evidence that they are fundamental designs rather than borrowed curiosities. From social-insect chemical communication: **stigmergic coordination and division of labor** (topic-concentration fields supporting quorum sensing and silence-as-signal, dual releaser/primer messages, need-proportional recruitment, sensory-filter specialization, live trail reinforcement, and a feedback path from the system's own aggregate activity into its chemistry). From the criticality of starling murmurations: **branching-ratio control** with arousal as the control parameter that holds the cluster network near its point of maximal responsiveness, plus a per-turn chemistry trajectory. Plasticity follows a graded, neuromodulator-scaled learning rule consistent with three-factor learning theory and the inverted-U of stress on memory. Several mechanisms remain too nascent to evaluate, and the central question of whether multi-agent structure improves on a single well-prompted model has not yet been subjected to controlled comparison. We present the architecture, its philosophical commitments, and an honest account of what the data does and does not indicate.
...
- 1,176 pytest tests across 57 test modules — including the coordination layer's silence state-machine transitions, slope-aware quorum, and self-limiting-feedback proofs
...
Mean LLM calls per turn: **3.38** (range 0–11). This is within the designed range of 3–5 for typical turns, with the mean ticking up from 3.23 at the prior snapshot as the self-directed task system and skill-disambiguation step fire on more turns. The 0 minimum confirms the template-match switch path is functional. The maximum of 11 exceeds the designed ceiling of 8 on a small number of autonomous task-completion turns, where the strategic_planner, follow_through loop, and ResultReporter cells stack additional integrator calls on top of the normal drafting pipeline.

Mean latency: **13.1 seconds** (range 0.8–83.1s). The mean has risen from 8.7s at the prior snapshot, and the maximum from 26.5s to 83.1s — both attributable to the same source as the call-count increase: autonomous multi-step task turns (planner plus a step-by-step follow-through loop) and the skill-selection LLM disambiguation step now appear in the dataset, and these are the longest turns. Short conversational turns remain in the 1–3s range. Latency remains the primary user-facing limitation; it is framed in the design as "deliberation time" rather than lag, with the real-time brain activation visualization serving as evidence of live processing rather than a loading state.
...
The 25.4% save rate is effectively flat from the 26.0% at the prior snapshot and consistent with sessions averaging 5.7 turns — within-session predictor history is thin at that length, which limits how far the rate can climb per session. Longer sessions are the primary lever for approaching the 30–50% design target.
...
Emotion distribution observed across all 393 turns: **curious (31%)**, neutral (30%), scattered (10%), thoughtful (9%), excitement (5%), confident (4%), alert-curious (4%), lively (2%), wistful (1%), engaged (1%), joy (1%), serene (1%), settled (1%), content (1%), with a single occurrence of `somber` (<1%). Fifteen distinct states are now observed, up from fourteen at the prior snapshot. The appraisal-based overrides (embarrassed, sympathetic, flirty, etc.) have still not been triggered in this dataset — sessions have been non-hostile and non-emotionally-charged — but the infrastructure is confirmed functional.

The most notable addition is **`somber`, the first negative-valence base state to appear** in any snapshot. Its single occurrence is too thin to interpret, but its appearance confirms that the low-valence region of the emotion lookup table is reachable from organic session dynamics, not only from the hostile/threat pathway that drives GABA. Until now every observed base state had been neutral or positive.

`scattered` at 10% remains the third most common state. It maps to a recognizable neuromod signature: elevated NE (above the scatter threshold) combined with reduced ACh and DA near its floor — an over-activated attentional system with insufficient reward signal, a kind of internally fragmented attention. This state is an NE overlay, not a base chemistry label: it emerges when the NE color-overlay rule fires on top of a low-DA, low-reward base state, and the now-logged NE values (§6.4) make that mechanism directly visible in the data. It correlates with the DA-floor sessions in the trajectory above. The non-canonical states collectively account for 39% of turns (up from 33%), reflecting a system whose emotional range has broadened as the chemistry explores more of its state space.
...
**Predict-and-surprise gating is stable and producing consistent savings.** 25.4% of turns are successfully gated — flat from the prior 26.0% and consistent with session-length constraints rather than mechanism failure. The `switch_suppressed_by_modulation` decision type (30 occurrences) shows the neuromodulator system influencing computation at the individual switch level, not only at the integrator-suppression level. This is the most granular evidence that chemistry is doing real computational work.

**The neuromodulator and hormonal dynamics are functional and showing longitudinal behavior.** All four core fast channels decline from early to late sessions (ACh 0.444 → 0.409, DA 0.410 → 0.344, GABA 0.080 → 0.030, Glu 0.339 → 0.232), a broad settling of arousal as the environment becomes familiar. The per-session trajectory runs neutral plateau → curious/high-ACh exploratory peak → diversification into confident/thoughtful/joy/lively → recent oscillation between DA-floor scattered and recovered states, with two independent signals (turn-level state and session-end self-model) in agreement. Separately, this is the first snapshot in which the slow hormonal tier (5HT, CORT, OXT, AEA) and NE produce logged values, and they are internally coherent — cortisol at its floor, oxytocin beginning to accumulate, serotonin holding a stable baseline. The ACh longitudinal decline is notable: if it reflects genuine familiarity accumulation rather than topic-content bias, it is the first evidence of long-horizon environmental adaptation in the neuromodulator system.

**Memory recall is active and growing.** Contributing to 35.9% of turns (up from 33.1%) means the hippocampus is a live participant in the pipeline, not merely an accumulating store.
...
**Hebbian plasticity producing emergent behavioral style.** Three consolidation passes now show consistent reinforcement of the same core pathway (`temporal.understanding_integrator → frontal.executive` strengthened in all three), with plasticity modulators correctly elevated on engaged sessions, and the third pass — the first under the refined outcome formula — beginning to reinforce a new path from the preferred drafter into commitment extraction. The theoretical prediction — that preferred drafters, recall paths, and switch orderings will emerge from reinforcement over many sessions — is testable but not yet tested. The refined formula's expected effect (long-term depression, i.e. edge weakening) has not yet appeared, because the one session scored under it was benign. This is a months-long experiment.

**The emotion evolution producing differentiated behavior.** 15 distinct states are now observed; `scattered` at 10% remains the most notable recurring signal, correlating with the DA-floor sessions, and `somber` has appeared as the first negative-valence base state. Deliberate emotional expression (§4.9) adds a further dimension: the system can perform an emotional register for communicative effect without modifying its chemical state. Whether either authentic emotion or deliberate expression produces measurably different response quality on matched inputs requires controlled comparison not yet conducted.
...
**Latency.** Mean 13.1 seconds (up from 8.7s) is workable but not conversational, and the rise reflects new capability rather than regression: autonomous multi-step task turns and the skill-disambiguation step are now in the dataset and are the longest turns, pushing the maximum to 83.1s. The system was designed with the expectation that latency would be a primary UX limitation, framed as "deliberation" rather than lag. Shorter conversational turns still achieve sub-second to 2-second latency; complex multi-drafter and autonomous-task turns drive the mean up. This is a fundamental constraint of sequential LLM calls on the critical path.
...
The dataset (393 turns, 69 sessions) supports the following conclusions:

1. **The cost model is validated.** Mean 3.38 LLM calls/turn is within the designed range; the maximum of 11 on autonomous task-completion turns is consistent with the self-directed task system stacking planner, follow-through, and ResultReporter calls.
2. **Predict-and-surprise gating is stable** at 25.4% of turns (flat from 26.0%); the plateau reflects session-length constraints rather than mechanism failure. The `switch_suppressed_by_modulation` decision type (30 occurrences) shows chemistry influencing computation at the individual switch level.
3. **Neuromodulator dynamics are showing long-horizon behavior** — all four core fast channels decline from early to late sessions (ACh 0.444 → 0.409, DA 0.410 → 0.344, GABA and Glu in parallel). If this reflects familiarity accumulation rather than topic bias, it is the first evidence of genuine environmental adaptation.
4. **The slow hormonal tier and NE now produce logged values for the first time** — serotonin holds a stable baseline, cortisol rests at its floor, oxytocin has begun to accumulate, and NE sits mostly in its optimal band — giving the first empirical readout of the nine-channel chemical system.
5. **Memory recall is active** on 35.9% of turns (up from 33.1%), with the `reuse_recent_recall` efficiency mechanism visible in the data (13 occurrences).
6. **Hebbian plasticity is running** with three consolidation passes; the same core edge is reinforced across all three, and the third — the first under the refined outcome formula — updated more edges and began reinforcing a new drafter→commitment-extraction path.
7. **The emotional vocabulary has expanded** to 15 distinct observed states; `scattered` (10%) correlates with the DA-floor sessions, and `somber` has appeared as the first negative-valence base state.
8. **Seven significant capabilities are operational**: self-directed task system, REM-style DMN thought consolidation, inner monologue integration, deliberate emotional expression, DMN autonomy with urgency-ranked deferred thoughts, a background resource policy for autonomous computation, and a dynamic per-turn reasoning-framework selector (a ~380-skill self-reflection library) now visible in the decision log.
9. **The codebase has been substantially refactored** for maintainability: BrainSession class with mixin decomposition, HebbianUpdater, ToolDispatcher, and companion prompt files; 1,176 tests across 57 modules pass.
10. **The coordination layer incorporates two convergent natural designs** beyond the brain's own chemistry: stigmergic coordination and division of labor (quorum/silence, need-proportional recruitment, sensory-filter specialization, live trail reinforcement, and aggregate-state chemistry feedback), drawn from social-insect communication, and branching-ratio criticality control with arousal as its control parameter, drawn from flock dynamics. They are adopted not as biomimetic novelties but because patterns that evolve independently in brains, insect colonies, and bird flocks are evidence of fundamental designs for decentralized coordination — the same problem this architecture solves. Their emergent payoff is the central subject of next-phase evaluation. Plasticity has additionally moved from a binary skip to a graded, neuromodulator-scaled per-turn rule.
11. **The observability infrastructure is complete** and enables the analysis above.
...
The most honest characterization of the current state: the substrate is active, behavioral patterns are accumulating, and the system is operational across all designed mechanisms. The decline of all four core fast neuromodulators across 69 sessions (ACh 0.444 → 0.409, DA 0.410 → 0.344) is the most substantive empirical signal in the dataset — a longitudinal trend, not a within-session artifact, suggesting the system is tracking genuine environmental change as its world becomes familiar. This is also the first snapshot in which the slow hormonal tier and NE produce logged values, giving a first coherent readout of the full nine-channel chemical system. The `switch_suppressed_by_modulation` decision type shows chemistry operating at the finest granularity the architecture permits. The emotional vocabulary is diverse and now includes a first negative-valence base state (`somber`), with DA-floor `scattered` sessions emerging as a recognizable regime. None of these yet constitute "interesting emergent behavior" in the strong sense the design literature uses — but the evidence that the mechanisms are doing real computational work, not merely executing fixed logic, is present in the data.

Framed against the field's prevailing ambition, the contribution is best read not as a step toward artificial *general* intelligence but as a working prototype of artificial *emotional* intelligence (AEI): a system whose intelligence is organized around affect as a real, persistent, computationally load-bearing state rather than around breadth of task capability. Whether that design point produces meaningfully different longitudinal behavior is the question the architecture exists to answer.
...
*Data source: `eval/turns.jsonl` (393 turns, 69 sessions, as of 2026-05-29)*  
