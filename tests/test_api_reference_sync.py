...
The prose half — that every route is actually DOCUMENTED, and that the rendered
docs payload is safe and complete — lives in tests/test_api_docs.py, alongside the
guide it guards. Two tests used to live here that regex-scraped a hand-maintained
endpoint index out of docs/API.md; they were checking a second copy of data the
router already generates, and the appendix that fed them is gone.
"""
