...
class TestAuditoryCluster:
    async def test_silence_gate_no_publish(self, tmp_path, monkeypatch):
        ...
        await bus.publish_dict(
            "auditory.raw_audio",
            {
                "audio_bytes": _silence_bytes(2.0),
                "sample_rate": SR,
                "dtype": "int16",
                "duration_s": 2.0,
                "channels": 1,
    ...
    async def test_prosody_published_for_tone(self, tmp_path, monkeypatch):
        ...
        await bus.publish_dict(
            "auditory.raw_audio",
            {
                "audio_bytes": _sine_bytes(440.0, duration=3.0),
                "sample_rate": SR,
                "dtype": "int16",
                "duration_s": 3.0,
                "channels": 1,
...
class TestLabelProsodyTone:
    ...
    def test_energetic_high_energy_and_rate(self):
        ...
        assert (
            label_prosody_tone(self._features(energy_mean=0.15, speech_rate_hz=5.0)) == "energetic"
        )
    ...
    def test_uncalibrated_baseline_uses_universal_thresholds(self):
        ...
        assert (
            label_prosody_tone(self._features(jitter=0.05, shimmer=0.12), baseline=baseline)
            == "stressed"
        )
    ...
    def test_calibrated_baseline_raises_stressed_threshold(self):
        ...
        assert (
            label_prosody_tone(self._features(jitter=0.04, shimmer=0.06), baseline=baseline)
            == "calm"
        )
        # Well above their baseline → stressed
        assert (
            label_prosody_tone(self._features(jitter=0.09, shimmer=0.12), baseline=baseline)
            == "stressed"
        )
    ...
    def test_floor_prevents_threshold_below_universal(self):
        ...
        assert (
            label_prosody_tone(self._features(jitter=0.04, shimmer=0.12), baseline=baseline)
            == "stressed"
        )
...
class TestSpeakerStoreProsodyBaseline:
    ...
    def test_running_mean_converges(self, tmp_path):
        ...
        sid = store.enroll("Russ", np.random.randn(192).astype(np.float32))
        for _ in range(20):
            store.update_prosody_baseline(
                sid, {"jitter": 0.04, "shimmer": 0.06, "energy_mean": 0.08, "f0_std": 22.0}
            )
        bl = store.get_prosody_baseline(sid)
    ...
    def test_zero_values_not_included_in_mean(self, tmp_path):
        ...
        store.update_prosody_baseline(
            sid, {"jitter": 0.04, "shimmer": 0.06, "energy_mean": 0.08, "f0_std": 20.0}
        )
        # Second update with zero jitter/shimmer (unvoiced frame — should be skipped)
        store.update_prosody_baseline(
            sid, {"jitter": 0.0, "shimmer": 0.0, "energy_mean": 0.07, "f0_std": 19.0}
        )
        bl = store.get_prosody_baseline(sid)
    ...
    def test_persists_across_reload(self, tmp_path):
        ...
        for _ in range(12):
            store.update_prosody_baseline(
                sid, {"jitter": 0.035, "shimmer": 0.055, "energy_mean": 0.09, "f0_std": 24.0}
            )
        store2 = SpeakerStore(profiles_dir=tmp_path)
