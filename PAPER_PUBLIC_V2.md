...
**Intent detection (embedding-based, learned).** The fast-path *intent* gates — whether the user is referring to the entity itself or using it as a cognitive tool that recalls shared history — are detected by semantic similarity rather than a fixed keyword list. Each intent keeps a small bank of exemplar phrasings; the input is embedded once by a local model (off the language-model critical path) and matched against the bank, so paraphrases a literal list would miss still register. The bank seeds from a handful of canonical phrases and grows with use: when the understanding integrator confirms an intent the cheap match missed, that phrasing is added as an exemplar, so detection converges on how a given user actually speaks and the expensive judgment is needed less over time. A literal seed match remains the fast path and the fallback when no embedder is available. This learned, paraphrase-robust mechanism is the direction the affect lexicon above is moving toward — replacing static word lists with detectors that improve from experience.
